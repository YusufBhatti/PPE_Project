  subroutine cosp_dsd(Q,Re,Np,D,N,nsizes,dtype,rho_a,tk, &
             dmin,dmax,apm,bpm,rho_c,p1,p2,p3)

  USE mo_kind,		ONLY: wp
  USE mo_cosp_array_lib
  USE mo_cosp_math_lib 
...
! ----- INPUTS -----  
  ...
  real(wp), intent(in)  :: Q,Re,Np,D(nsizes)
  real(wp), intent(in)  :: rho_a,tk,dmin,dmax,rho_c,p1,p2,p3
    
  real(wp), intent(inout) :: apm,bpm  
  
! ----- OUTPUTS -----

  real(wp), intent(out) :: N(nsizes)
...
! :: vu = distribution width parameter
  ...
  case(1)  
    ...
    D0 = 2.0_wp*Re*gamma(vu+2)/gamma(vu+3)
   
        fc = ( &
             ((D*1E-6_wp)**(vu-1)*exp(-1*D/D0)) / &
             (apm*((D0*1E-6_wp)**(vu+bpm))*gamma(vu+bpm)) &
         ) * 1E-12_wp

        N = fc*rho_a*(Q*1E-3_wp)
        
      elseif( p2+1 > 1E-8_wp) then     ! use default value for MEAN diameter
    ...
    D0 = gamma(vu)/gamma(vu+1)*dm
      ...
      elseif(abs(p3+1) > 1E-8_wp)  then! use default number concentration
    
        local_np = p1 ! total number concentration / pa check
   
    tmp1 = (Q*1E-3_wp)**(1._wp/bpm)
      
        fc = (D*1E-6_wp / (gamma(vu)/(apm*local_np*gamma(vu+bpm)))** &
             (1._wp/bpm))**vu
         
        N = ( &
          (rho_a*local_np*fc*(D*1E-6_wp)**(-1._wp))/(gamma(vu)*tmp1**vu) * &
          exp(-1._wp*fc**(1._wp/vu)/tmp1) &
      ) * 1E-12_wp
...
! :: ld = slope parameter (um)
  ...
  case(2)
    ...
    fc = (ld*1E6_wp)**(1._wp+bpm)/(apm*gamma(1+bpm))* &
               exp(-1._wp*(ld*1E6_wp)*(D*1E-6_wp))*1E-12_wp
     
        N = fc*rho_a*(Q*1E-3_wp)
        
    elseif (abs(p1+1) > 1E-8_wp) then
    ...
    ! use N0 default value
        ...
        fc = ((apm*gamma(1._wp+bpm)*N0)**tmp1)*(D*1E-6_wp)
  
        N = ( &
            N0*exp(-1.*fc*(1._wp/(rho_a*Q*1E-3_wp))**tmp1) &
    ) * 1E-12_wp

    elseif (abs(p2+1) > 1E-8_wp) then
    ...
    !     used default value for lambda 
      ...
      ld = 1220*10._wp**(-0.0245_wp*tc)*1E-6_wp
      N0 = ((ld*1E6_wp)**(1+bpm)*Q*1E-3_wp*rho_a)/(apm*gamma(1+bpm))
...
!   :: br parameter
    if (abs(p1+2) < 1E-8_wp) then
!     :: if p1=-2, bhp is parameterized according to Ryan (2000),
!     :: applicatable to cirrus clouds
      if (tc < -30) then
        bhp = -1.75_wp+0.09_wp*((tc+273)-243.16_wp)
      elseif ((tc >= -30) .and. (tc < -9)) then
        bhp = -3.25_wp-0.06_wp*((tc+273)-265.66_wp)
      else
        bhp = -2.15_wp
      endif
    elseif (abs(p1+3) < 1E-8_wp) then      
!     :: if p1=-3, bhp is parameterized according to Ryan (2000),
!     :: applicable to frontal clouds
      if (tc < -35) then
        bhp = -1.75_wp+0.09_wp*((tc+273)-243.16_wp)
      elseif ((tc >= -35) .and. (tc < -17.5_wp)) then
        bhp = -2.65_wp+0.09_wp*((tc+273)-255.66_wp)
      elseif ((tc >= -17.5_wp) .and. (tc < -9)) then
        bhp = -3.25_wp-0.06_wp*((tc+273)-265.66_wp)
      else
        bhp = -2.15_wp
      endif    
...
!   :: Ar parameter
    dmin_mm = dmin*1E-3_wp
    dmax_mm = dmax*1E-3_wp
...
! :: D0 = particle diameter (um)
  ...
  case(4)
    ...
    if (Re>0) then
      ...
      rho_e = (6/pi)*apm*(D0*1E-6_wp)**(bpm-3)
      fc(1) = (6._wp/(pi*D0**3*rho_e))*1E12_wp
      N(1) = fc(1)*rho_a*(Q*1E-3_wp)
...
! :: log_sigma_g = ln(geometric standard deviation)
  ...
  case(5)
    if (abs(p1+1) < 1E-8_wp .or. Re>0 ) then

!     // rg, log_sigma_g are given
      log_sigma_g = p3
      tmp2 = (bpm*log_sigma_g)**2._wp
      if(Re.le.0) then 
        rg = p2
      else
    rg =Re*exp(-2.5_wp*(log_sigma_g**2))
         ...
         fc = 0.5_wp * ( &
         (1._wp/((2._wp*rg*1E-6_wp)**(bpm)*apm*(2._wp*pi)**(0.5_wp) * &
         log_sigma_g*D*0.5_wp*1E-6_wp)) * &
         exp(-0.5_wp*((log(0.5_wp*D/rg)/log_sigma_g)**2._wp+tmp2)) &
         ) * 1E-12_wp
                
      N = fc*rho_a*(Q*1E-3_wp)
      
    elseif (abs(p2+1) < 1E-8_wp .or. Np>0) then
...
!     // Np, log_sigma_g are given    
      ...
      log_sigma_g = p3
      N0 = local_np*rho_a
      tmp1 = (rho_a*(Q*1E-3_wp))/(2._wp**bpm*apm*N0)
      tmp2 = exp(0.5_wp*bpm**2._wp*(log_sigma_g))**2._wp      
      rg = ((tmp1/tmp2)**(1/bpm))*1E6_wp
      
      N = 0.5_wp*( &
        N0 / ((2._wp*pi)**(0.5_wp)*log_sigma_g*D*0.5_wp*1E-6_wp) * &
    exp((-0.5_wp*(log(0.5_wp*D/rg)/log_sigma_g)**2._wp)) &
    ) * 1E-12_wp      
