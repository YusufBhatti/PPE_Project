...
!sschr????
  ...
  !>>DN
  USE mo_hammoz_aerocom_diags,  ONLY: lTraj
  USE mo_hammoz_aerocom_Traj,   ONLY: emissa, emissc
  !<<DN
...
! note: id_ss = 0 should not occur here. Let it crash if it does ...
    ...
    !<<dod
    !>>DN
    IF (lTraj) emissa(1:kproma,krow)=emissa(1:kproma,krow)+zmassf1(1:kproma)
    IF (lTraj) emissc(1:kproma,krow)=emissc(1:kproma,krow)+zmassf2(1:kproma)
    !<<DN
