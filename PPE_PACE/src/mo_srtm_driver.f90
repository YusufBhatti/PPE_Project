...
MODULE mo_srtm_driver
  ...
  USE mo_srtm_setup,         ONLY : ngb, wavenum2, ssi_default, delwave
  USE mo_srtm_gas_optics,    ONLY : gpt_taumol, ih2o, ich4, ico2, io2, io3
  USE mo_rrtm_coeffs,        ONLY : srtm_coeffs
  USE mo_srtm_solver,        ONLY : srtm_solver, delta_scale, two_stream
  USE mo_cld_sampling,       ONLY : sample_cld_state
  USE mo_spec_sampling,      ONLY : spec_sampling_strategy, get_gpoint_set
  ...
  INTEGER :: i
#if defined (__PGI)
  REAL (wp)                 :: frc_par_array(nbndsw)
  REAL (wp)                 ::       frc_vis(nbndsw)
#else
...
CONTAINS
  ...
  SUBROUTINE srtm( kproma                                                    , &
       ...
       &  cld_labels0     ,cld_labels      ,flxd_sw_ct      ,flxu_sw_ct      , &
    ...
    USE mo_cloudtypes, ONLY: nclusters, nclusters_matus
    USE mo_p3_fields,  ONLY: lspeedrun
    ...
    INTEGER, INTENT(in)   :: cld_labels(kbdim,klev)  !< original cloud labels
    INTEGER, INTENT(in)   :: cld_labels0(kbdim,klev) !< matus cloud aggregate labels
    REAL(wp), INTENT(out) :: flxd_sw_ct(kbdim,klev+1,2*nclusters+nclusters_matus)
    REAL(wp), INTENT(out) :: flxu_sw_ct(kbdim,klev+1,2*nclusters+nclusters_matus)
    ...
    INTEGER :: nc   !< cloud type cluster index
    INTEGER :: nnc  !< cloud type cluster index
    ...
    REAL(wp) :: mask(kbdim,klev)              ! grid-boxes to be considered for labelling
    REAL(wp) :: zlabels(kbdim,klev)           ! local copy of labels
    ...
    LOGICAL  :: lclearsky ! switch between all-sky and clear sky as reference for types CRE
    ...
    ! --------------------------------
#if defined (__PGI)
    frc_par_array(:) = (/ (0.0_wp, i = 1, 8), 0.533725_wp, 1.0_wp, 0.550164_wp, (0.0_wp, i = 12, nbndsw) /) 
    frc_vis(:) = MAX(0.0_wp, MIN(1.0_wp, (/ ((wavenum2(i+jpb1-1) - nir_vis_boundary) / delwave(i+jpb1-1), i = 1, nbndsw) /) ))
#endif
    !
...
!IBM* ASSERT(NODEPS)
    ...
    flxd_sw_ct (1:kproma,1:klev+1,:) = 0.0_wp
    flxu_sw_ct (1:kproma,1:klev+1,:) = 0.0_wp
...
!IBM* ASSERT(NODEPS)
      ...
      !
      ! Individual contributions per cloud type
      !
      IF(.NOT. lspeedrun) THEN
         DO nc=1,2*nclusters+nclusters_matus
            ! cloud radiative effect: all-sky - (all-sky - type)
            IF(nc < nclusters + 1) THEN
               zlabels(1:kproma,:) = cld_labels0(1:kproma,:)
               nnc = nc
               lclearsky = .FALSE.
            ! cloud radiative effect: type - clear-sky
            ELSE IF(nc < 2*nclusters + 1) THEN
               zlabels(1:kproma,:) = cld_labels0(1:kproma,:)
               nnc = nc-nclusters
               lclearsky = .TRUE.
            ! cloud radiative effect: type - clear-sky -> aggregated (Matus & L'Ecuyer 2017)
            ELSE
               zlabels(1:kproma,:) = cld_labels(1:kproma,:)
               nnc = nc-2*nclusters
               lclearsky = .TRUE.
            END IF

            ! single out each cloud type by calculating radiation
            IF(lclearsky) THEN
               mask(1:kproma,:) = MERGE(1._wp, 0._wp, zlabels(1:kproma,:) == nnc)
            ELSE
               mask(1:kproma,:) = MERGE(1._wp, 0._wp, zlabels(1:kproma,:) /= nnc)
            ENDIF ! lclearsky
            mask(1:kproma,:) = MERGE(mask(1:kproma,:), 0._wp, zlabels(1:kproma,:) > 0) ! only cloudy

            ztaut(1:kproma,:,ig) = ztaur(1:kproma,:,ig) + ztaua(1:kproma,:,ig) + ztaug(1:kproma,:,ig) & 
                                + mask(1:kproma,:)*ztauc(1:kproma,:,ig)
            zomgt(1:kproma,:,ig) = ztaur(1:kproma,:,ig) + ztaua(1:kproma,:,ig)*zomga(1:kproma,:,ig) & 
                                + mask(1:kproma,:)*ztauc(1:kproma,:,ig)*zomgc(1:kproma,:,ig) 
            zasyt(1:kproma,:,ig) =(zasya(1:kproma,:,ig)*zomga(1:kproma,:,ig)*ztaua(1:kproma,:,ig) & 
                                + mask(1:kproma,:)*zasyc(1:kproma,:,ig)*zomgc(1:kproma,:,ig)*ztauc(1:kproma,:,ig))&
                                  / zomgt(1:kproma,:,ig)
            zomgt(1:kproma,:,ig) = zomgt(1:kproma,:,ig) / ztaut(1:kproma,:,ig)
            ...
            CALL two_stream(kproma, kbdim       ,klev                         , &
                 ...
                 Rdir(:,:)      ,Rdif(:,:)      ,Tdir(:,:)      ,Tdif(:,:)) 
            !
            ! Compute fluxes using cloud optical properties
            ! 
            CALL srtm_solver(kproma, kbdim, klev, &
                 & albdif(:,  ig), albdir(:, ig), cossza(:), ztaut(:,:,ig), &
                 & Rdir(:,:), Rdif(:,:), Tdir(:,:), Tdif(:,:),              &
            ...
            ! Accumulate broadband (flx) clear-sky fluxes
            !
            flxu_sw_ct(1:kproma,1:klev+1,nc) = flxu_sw_ct(1:kproma,1:klev+1,nc) &
                 + gpt_scaling * SPREAD(zincflx(1:kproma),DIM=2, NCOPIES=klev+1) * zgpcu(1:kproma,1:klev+1)
            flxd_sw_ct(1:kproma,1:klev+1,nc) = flxd_sw_ct(1:kproma,1:klev+1,nc) &
                 + gpt_scaling * SPREAD(zincflx(1:kproma),DIM=2, NCOPIES=klev+1) * zgpcd(1:kproma,1:klev+1)
         END DO !nc
