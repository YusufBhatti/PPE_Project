...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE ham_define_tracer
    ...
    USE mo_p3_fields,         ONLY: idt_qirim, idt_birim, idt_qihet, idt_qiliq, idt_qioliq, &
...
!### not sure about this logic: please test!!
    ...
    ELSE
       ...
       iwrite = ON
    ENDIF
    CALL new_tracer('QIRIM', 'HAM', ierr=ierr,     &
                     idx=idt_qirim,                &
                     units='kg kg-1',              &
                     table=131,                    &
                     code=133,                     &
    ...
    IF (ierr /= OK) THEN
      WRITE(message_text,'(a,i0)') 'new_tracer QIRIM returned error code ',ierr
      CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('BIRIM', 'HAM', ierr=ierr,     &
                     idx=idt_birim,                &
                     units='m3',                   &
                     table=131,                    &
                     code=133,                     &
                     ...
                     nvdiff=ivdiff,                &
                     nint=iint,                    &
                     longname='riming volume')

    IF (ierr /= OK) THEN
      WRITE(message_text,'(a,i0)') 'new_tracer BIRIM returned error code ',ierr
      CALL message('ham_define_tracer', message_text, level=em_error)
    END IF
    ...
    ELSE
       ...
       iwrite = ON
    ENDIF !lspeedrun
    CALL new_tracer('QIHET', 'HAM', ierr=ierr,     &
                     idx=idt_qihet,                &
                     units='kg kg-1',                   &
                     table=131,                    &
                     code=134,                     &
                     ...
                     nvdiff=ivdiff,                  &
                     nint=iint,                    &
                     longname='heterogeneously formed ice')

    IF (ierr /= OK) THEN
      WRITE(message_text,'(a,i0)') 'new_tracer QIHET returned error code ',ierr
      CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('QILIQ', 'HAM', ierr=ierr,     &
                     idx=idt_qiliq,                &
                     units='kg kg-1',                   &
                     table=131,                    &
                     code=135,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='ice from liquid')

    IF (ierr /= OK) THEN
      WRITE(message_text,'(a,i0)') 'new_tracer QILIQ returned error code ',ierr
      CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('QIOLIQ', 'HAM', ierr=ierr,     &
                     idx=idt_qioliq,                &
                     units='kg kg-1',                   &
                     table=131,                    &
                     code=136,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='ice initiated by liquid')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer QIOLIQ returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('QSRC', 'HAM', ierr=ierr,     &
                     idx=idt_qsrc,                &
                     units='kg kg-1',                   &
                     table=131,                    &
                     code=137,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='accumulated water source terms')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer QSRC returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF
    CALL new_tracer('QPRC', 'HAM', ierr=ierr,     &
                     idx=idt_qprc,                &
                     units='kg kg-1',                   &
                     table=131,                    &
                     code=138,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='accumulated water precip sink')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer QPRC returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('NIHET', 'HAM', ierr=ierr,     &
                     idx=idt_nihet,                &
                     units='kg-1',                   &
                     table=131,                    &
                     code=138,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='Het. origin ice number')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer NIHET returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('NIHOM', 'HAM', ierr=ierr,     &
                     idx=idt_nihom,                &
                     units='kg-1',                   &
                     table=131,                    &
                     code=138,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='Hom. origin ice number')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer NIHOM returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('NINUC', 'HAM', ierr=ierr,     &
                     idx=idt_ninuc,                &
                     units='kg kg-1',                   &
                     table=131,                    &
                     code=138,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='Nuc. origin ice number')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer NINUC returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF

    CALL new_tracer('NIDET', 'HAM', ierr=ierr,     &
                     idx=idt_nidet,                &
                     units='kg-1',                   &
                     table=131,                    &
                     code=138,                     &
                     ...
                     nvdiff=ivdiff,                    &
                     nint=iint,                      &
                     longname='Detrained ice number')

    IF (ierr /= OK) THEN
       WRITE(message_text,'(a,i0)') 'new_tracer NIDET returned error code ',ierr
       CALL message('ham_define_tracer', message_text, level=em_error)
    END IF
