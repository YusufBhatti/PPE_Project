...
!<<SF
  USE mo_remos,     ONLY: drieff_optics
...
!>>SF
    USE mo_param_switches, ONLY: ncd_activ, t_zinhomi
!<<SF
   ...
   !>> RD circumvent standard tuning!
    ...
    IF (p_parallel_io) THEN
      ...
      CALL io_inq_varid(optical_tbl%file_id, 'wavenumber', varID)
      CALL io_get_vara_double(optical_tbl%file_id, varID, (/1/),  &
           (/n_mdl_bnds/), wavenumber)
      CALL io_inq_varid(optical_tbl%file_id, 'wavelength', varID)
      CALL io_get_vara_double(optical_tbl%file_id, varID, (/1/),  &
           (/n_mdl_bnds/), wavelength)
      CALL io_inq_varid(optical_tbl%file_id, 're_droplet', varID)
      CALL io_get_vara_double(optical_tbl%file_id, varID, (/1/),  &
           (/n_sizes/), re_droplet)
      CALL io_inq_varid(optical_tbl%file_id, 're_crystal', varID)
      CALL io_get_vara_double(optical_tbl%file_id, varID, (/1/),  &
           (/n_sizes/), re_crystal)

      CALL io_inq_varid(optical_tbl%file_id,"extinction_per_mass_droplet",varID)
      CALL io_get_vara_double(optical_tbl%file_id,varID,(/1,1/), &
           (/n_sizes,n_mdl_bnds/),z_ext_l )
      CALL io_inq_varid(optical_tbl%file_id,"co_albedo_droplet",varID)
      CALL io_get_vara_double(optical_tbl%file_id,varID,(/1,1/), &
           (/n_sizes,n_mdl_bnds/),z_coa_l )
      CALL io_inq_varid(optical_tbl%file_id,"asymmetry_factor_droplet",varID)
      CALL io_get_vara_double(optical_tbl%file_id,varID,(/1,1/), &
           (/n_sizes,n_mdl_bnds/),z_asy_l )

      CALL io_inq_varid(optical_tbl%file_id,"extinction_per_mass_crystal",varID)
      CALL io_get_vara_double(optical_tbl%file_id,varID,(/1,1/), &
           (/n_sizes,n_mdl_bnds/),z_ext_i )
      CALL io_inq_varid(optical_tbl%file_id,"co_albedo_crystal",varID)
      CALL io_get_vara_double(optical_tbl%file_id,varID,(/1,1/), &
           (/n_sizes,n_mdl_bnds/),z_coa_i )
      CALL io_inq_varid(optical_tbl%file_id,"asymmetry_factor_crystal",varID)
      CALL io_get_vara_double(optical_tbl%file_id,varID,(/1,1/), &
...
!>>SF
    WRITE(message_text,*) 'Exponent for variable liquid-cloud inohomgeneity: zinpar= ',zinpar
    CALL message('setup_cloud_optics', message_text, level=em_param)
    WRITE(message_text,*) 'Ice-cloud inohomogeneity factor: zinhomi= ',zinhomi
    CALL message('setup_cloud_optics', message_text, level=em_param)
!<<SF
...
!>>SF
    USE mo_cloud_micro_2m, ONLY: eff_ice_crystal_radius, breadth_factor
    USE mo_cloud_utils,    ONLY: effective_ice_radius
    USE mo_param_switches, ONLY: ncd_activ
    USE mo_activ,          ONLY: idt_icnc
    USE mo_p3_fields,      ONLY: idt_birim, idt_qirim
!<<SF
    INTEGER, INTENT (IN)    :: &
    ...
    REAL (wp), INTENT (IN)  :: &
!>>SF
         zicnc(kbdim,klev),    & !< Ice crystal number concentration (ICNC), used value [1/m3]
!<<SF
         zlwp(kbdim,klev),     & !< liquid water path
         ziwp(kbdim,klev),     & !< ice water path
         pxtm1(kbdim,klev,ktrac),  & !< tracers for ice effective radius
         xm_ice(kbdim,klev),   & !< ice mmr for ice effective radius
...
!>>SF
         zrieff(kbdim),      & !< intermediate result for effective ice crystal rad.
         zri                   !< intermediate result for effective ice crystal rad.
!<<SF
    REAL(wp), DIMENSION(kbdim) :: ztmp1, ztmp2, ztmp3
    ...
    DO jk=1,klev
       ...
       !>>SF #265: correct breadth factor for 2-moment scheme
       IF (ncd_activ /= 0) THEN !2-moment scheme for liq phase
          zkap(:) = breadth_factor(kbdim, kproma, zcdnc(:,jk))
       ENDIF
       !<<SF #265

      !>>SF
      IF (nic_cirrus == 0) THEN
         zrieff(1:kproma) = 83.8_wp*ziwc(1:kproma,jk)**0.216_wp
      ELSE
         !SF #176: now the plate assumption is used for all temperatures:
         ! zrieff(:) = eff_ice_crystal_radius(kbdim, kproma, ziwc(:,jk), zicnc(:,jk))
         ! eff_ice_crystal_radius returns rieff in um!
         ! zrieff(:) = 1e6_wp*effective_ice_radius(kbdim, kproma, ziwc(:,jk), pqirim(:,jk), &
         !                                                     zicnc(:,jk), pbirim(:,jk))
         ! zrieff(1:kproma) = 1.e6_wp*drieff_optics(1:kproma,jk,krow)
!>>DN bugfix
!         ztmp1(:) = pxtm1(1:kproma,jk,idt_qirim)
!         ztmp2(:) = pxtm1(1:kproma,jk,idt_icnc)
!         ztmp3(:) = pxtm1(1:kproma,jk,idt_birim)
!         zrieff(:) = 1.e6_wp*effective_ice_radius(kbdim, kproma, xm_ice(:,jk), ztmp1, ztmp2, ztmp3)
         jkb = klev+1-jk
         ztmp1(1:kproma) = pxtm1(1:kproma,jkb,idt_qirim)
         ztmp2(1:kproma) = pxtm1(1:kproma,jkb,idt_icnc)
         ztmp3(1:kproma) = pxtm1(1:kproma,jkb,idt_birim)
         zrieff(:) = 1.e6_wp*effective_ice_radius(kbdim, kproma, xm_ice(:,jkb), ztmp1, ztmp2, ztmp3)
!<<DN bugfix
         ! zrieff(1:kproma) = 83.8_wp*ziwc(1:kproma,jk)**0.216_wp
         drieff_optics(1:kproma,jk,krow) = 1.e-6_wp*zrieff(1:kproma)
      ENDIF
      !<<SF
      ...
      DO jl=1,kproma
        IF (icldlyr(jl,jk)==1 .AND. (zlwp(jl,jk)+ziwp(jl,jk))>ccwmin) THEN
          ...
          re_crystals = MAX(reimin,MIN(reimax,zrieff(jl)))
          !<<SF

          re_droplets = MAX(relmin,MIN(relmax,zfact*zkap(jl)*(zlwc(jl,jk)    &
               &                             /MAX(1._wp,zcdnc(jl,jk)))**(1.0_wp/3.0_wp)))
