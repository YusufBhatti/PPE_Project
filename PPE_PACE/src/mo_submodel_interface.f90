...
!>>SF
  ...
  USE mo_p3_fields,   ONLY: p3_init, my_p3_init
!<<SF
...
!--mgs
    ...
    USE mo_remos,            ONLY: init_remos
    USE mo_cloudtypes,       ONLY: init_clouds
    USE mo_histogram,        ONLY: init_histos
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE cloud_subm_1(                                               &
   ...
   USE mo_hammoz_aerocom_HEaci, ONLY: ccn_inst, ccn1_inst, ccn3_inst, ccn5_inst, ccn10_inst
   USE mo_activ,                ONLY: swat
   USE mo_param_switches,       ONLY: ac_scale_activation, lMMPPE
   !<<DN
            ...
            CASE(HAM_M7)
               ...
               IF(lHEaci.AND.lHEaci_activ)THEN
                  ...
                  CALL ham_activ_abdulrazzak_ghan(kproma, kbdim, klev, krow, ktdia, &
                                               ...
                                               zw, zwpdf, za, zb, zrdry,         &
                                               znact, zfracn, zsc, zrc, zsmax)
                  swat(1:kproma,:,krow) = 0.010_wp
                  CALL ham_activ_abdulrazzak_ghan(kproma, kbdim, klev, krow, ktdia, &
                                               ccn10_inst(:,:,krow), pesw, prho,             &
                                               pxtm1, ptm1, papm1, pqm1,         &
                                               zw, zwpdf, za, zb, zrdry,         &
                                               znact, zfracn, zsc, zrc, zsmax)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE cloud_subm_3(                                 &
    ...
    USE mo_p3_fields,      ONLY: nconstnc
    !<<SF
#endif
    ...
    IF (laerocom_diag) THEN
       IF (lHEaci) THEN
          ...
          cdr3d(1:kproma,:,krow) = MERGE(prleff(1:kproma,:)*0.000001_wp,0._wp,&
               ...
               .AND.cdnc3d(1:kproma,:,krow)>nconstnc))
