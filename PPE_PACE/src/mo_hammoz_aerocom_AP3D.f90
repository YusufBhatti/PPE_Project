...
MODULE mo_hammoz_aerocom_AP3D
  ...
  REAL(dp), PUBLIC, POINTER :: ec550dryaer_int(:,:)  => NULL()
  REAL(dp), PUBLIC, POINTER :: abs550dryaer_int(:,:)  => NULL()
  REAL(dp), PUBLIC, POINTER :: ec550dryaer(:,:)  => NULL()
  REAL(dp), PUBLIC, POINTER :: abs550dryaer(:,:)  => NULL()
...
CONTAINS
  ...
  SUBROUTINE construct_AP3D_stream
    ...
    CALL add_stream_element (acp3d, 'ec550dryaer', ec550dryaer, &
        longname = 'Surface dry aerosol extinction at 550 nm', &
        units = 'm-1', &
        laccu = .TRUE., &
        lpost = .TRUE., &
        lrerun = .TRUE. )
    
    CALL add_stream_element (acp3d, 'abs550dryaer', abs550dryaer, &
        longname = 'Surface dry aerosol absorption at 550 nm', &
        units = 'm-1', &
        laccu = .TRUE., &
        lpost = .TRUE., &
        lrerun = .TRUE. )
    
    CALL add_stream_element (acp3d, 'ec550dryaer_int', ec550dryaer_int, &
        longname = 'Surface dry aerosol integrated extinction at 550 nm', &
        units = 'm-1', &
        laccu = .FALSE., &
        lpost = .FALSE., &
        lrerun = .FALSE. )
    
    CALL add_stream_element (acp3d, 'abs550dryaer_int', abs550dryaer_int, &
        longname = 'Surface dry aerosol integrated absorption at 550 nm', &
        units = 'm-1', &
        laccu = .FALSE., &
        lpost = .FALSE., &
        lrerun = .FALSE. )
  ...
  SUBROUTINE update_AP3D_diags(kproma, kbdim, klev, krow)
    ...
    ec550dryaer(1:kproma,krow)  = ec550dryaer(1:kproma,krow)  + &
         ec550aer_int(1:kproma,krow)/vphysc%grheightm1(1:kproma,klev,krow)*delta_time
    abs550dryaer(1:kproma,krow) = abs550dryaer(1:kproma,krow) + &
         abs550aer_int(1:kproma,krow)/vphysc%grheightm1(1:kproma,klev,krow)*delta_time
