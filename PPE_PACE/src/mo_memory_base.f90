...
MODULE mo_memory_base
  ...
  PUBLIC :: default_output         ! logical to allow easy disabling of streams
...
!  INTEGER      ,PARAMETER    :: maxstr   = 50    ! max number of output streams
  INTEGER      ,PARAMETER    :: maxstr   = 70    ! max number of output streams
!<<DN
  ...
  LOGICAL :: default_output = .true. ! enable output for standard streams
...
!==============================================================================
  ...
  CHARACTER(len=*)   ,INTENT(in)           :: name      ! name of output stream
  INTEGER            ,INTENT(in) ,OPTIONAL :: filetype  ! 'GRIB' or 'NetCDF'
...
!==============================================================================
  ...
  SUBROUTINE default_stream_setting (stream ,units                            &
                           ,ldims ,gdims ,repr                                &
                           ,lpost ,laccu, ldivi, reset, lrerun ,contnorest    &
  ...
  LOGICAL          ,INTENT(in) ,OPTIONAL :: ldivi         ! divide by interval length flag
  ...
  LOGICAL          ,INTENT(in) ,OPTIONAL :: no_default    ! use no defaults
    ...
    CALL set_memory_info (stream% default_info                                &
     ...
     ,ndim=99                                                                 &
     ,lpost=lpost ,laccu=laccu , ldivi=ldivi, reset=reset ,lrerun=lrerun      &
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE set_stream_element_info (stream, name  ,longname ,units         &
                          ...
                          ,lpost ,laccu, ldivi, lmiss, missval, reset, lrerun       &
  ...
  LOGICAL          ,INTENT(in) ,OPTIONAL :: no_default    ! use no defaults
    ...
    REAL(dp) :: old_reset_value

    element => find_list_element (stream, name)

    IF (ASSOCIATED (element)) THEN

      ! Prepare check if field should be re-initialized.
      old_reset_value = element%field%info%reset
      ...
      ! Only re-initialize field if some value has actually changed
      ! by either default settings or explicit flag.
      IF (old_reset_value /= element%field%info%reset) THEN
...
!==============================================================================
  ...
  INTEGER          ,INTENT(in) ,OPTIONAL :: tracidx
    ...
    CALL assign_if_present (info% ldivi      ,ldivi)
...
!==============================================================================
  ...
  SUBROUTINE add_stream_element_4d (stream, name, ptr, ldims, gdims,       &
             ...
             lpost, laccu, ldivi, lmiss, missval, reset, lrerun, contnorest,      &
             table, code, bits, leveltype,                                 &
  ...
  LOGICAL          ,INTENT(in) ,OPTIONAL :: verbose       ! produce printout
    ...
    CALL set_memory_info (new_list_element% field% info                       &
     ...
     ,repr=repr ,lpost=lpost ,laccu=laccu, ldivi=ldivi, lmiss=lmiss, missval=missval       &
    ...
    IF (alloc) THEN
      ...
      stream%memory_used = stream%memory_used &
                 +INT(8*SIZE(new_list_element%field%ptr),i8)
    ELSE
    ...
    new_list_element%field%ptr = new_list_element%field%info%reset
...
!-----------------------------------------------------------------------------
  ...
  INTEGER          ,INTENT(in) ,OPTIONAL :: tracidx
    ...
    CALL set_memory_info (new_list_element% field% info                       &
     ...
     ,lpost=lpost ,laccu=laccu, ldivi=ldivi, lmiss=lmiss, missval=missval                  &
     ,reset=reset, lrerun=lrerun                                              &
     ,contnorest=contnorest ,table=table ,code=code ,bits=bits                &
     ,leveltype=leveltype ,dimnames=dimnames                                  &
...
!-----------------------------------------------------------------------------
  SUBROUTINE add_stream_element_2d (stream, name, ptr, ldims, gdims,       &
             ...
             lpost, laccu, ldivi, lmiss, missval, reset, lrerun, contnorest,      &
             table, code, bits, leveltype,                                 &
             dimnames, p4, no_default, verbose)
  !
  ! create (allocate) a new table entry
  ...
  LOGICAL          ,INTENT(in) ,OPTIONAL :: verbose       ! produce printout
    ...
    alloc = .NOT. PRESENT(p4)
    element%field%info%alloc = alloc
    ...
    IF (alloc) THEN
      ...
      IF (istat /= 0) THEN
        WRITE(message_text,'(a,a)') 'Could not allocate ', TRIM(name)
        CALL finish('add_stream_element(2d)', message_text)
      ENDIF
      stream%memory_used = stream%memory_used &
                 +8*SIZE(element%field%ptr)
    ELSE
      element%field%ptr => p4
    ENDIF
...
!-----------------------------------------------------------------------------
  SUBROUTINE add_stream_element_1d (stream, name, ptr, ldims, gdims, &
    ...
    REAL(dp)     ,OPTIONAL,POINTER      :: p4(:,:,:,:)   ! provided pointer
    ...
    LOGICAL      ,OPTIONAL,INTENT(in)   :: ldivi       ! divide by interval length flag
