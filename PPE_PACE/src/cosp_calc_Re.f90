  subroutine cosp_calc_Re(Q,Np,rho_a, &
...
! ----- INPUTS -----  
  ...
  real(wp), intent(in) :: Q,Np,rho_a
 
  integer, intent(in):: dtype
  real(wp), intent(in) :: dmin,dmax,rho_c,p1,p2,p3
...
! :: vu = distribution width parameter 
  ...
  case(1)  
    ...
    elseif (abs(local_p3+1) > 1E-8_wp) then
    ...
    if( Np.eq.0 .and. p2+1 > 1E-8_wp) then     ! use default value for MEAN diameter as first default
    ...
    D0 = gamma(vu)/gamma(vu+1)*dm
        ...
        if(Np.eq.0) then
            ...
            if( abs(p1+1) > 1E-8_wp ) then  !   use default number concentration 
    ...
    D0 = 1E6_wp * ( Q*1E-3_wp*gamma(vu)/(apm*local_Np*gamma(vu+bpm)) )**(1/bpm)  ! units = microns
...
! :: ld = slope parameter (um)
  ...
  case(2)
    ...
    if((abs(p1+1) > 1E-8_wp) ) then   ! N0 has been specified, determine ld
    
        N0 = p1
    tmp1 = 1._wp/(1._wp+bpm)
    ld = ((apm*gamma(1._wp+bpm)*N0)/(rho_a*Q*1E-3_wp))**tmp1
    ld = ld/1E6_wp                     ! set units to microns^-1
        
    elseif (abs(p2+1) > 1E-8_wp) then  ! lambda=ld has been specified as default
...
! :: log_sigma_g = ln(geometric standard deviation)
  ...
  case(5)
        ...
        if( abs(local_p3+1) > 1E-8_wp ) then
    ...
    ! get rg ... 
    if( Np.eq.0 .and. (abs(p2+1) > 1E-8_wp) ) then ! use default value of rg
            ...
            log_sigma_g = p3
            tmp1 = (Q*1E-3_wp)/(2._wp**bpm*apm*local_Np)
            tmp2 = exp(0.5_wp*bpm**2._wp*(log_sigma_g))**2._wp      
            rg = ((tmp1/tmp2)**(1/bpm))*1E6_wp
        endif
                
        Re = rg*exp(+2.5_wp*(log_sigma_g**2)) 
        
  end select
  
  end subroutine cosp_calc_Re
