...
module mo_psrad_interface
  ...
  USE mo_srtm_setup,      ONLY: setup_srtm
  ...
  USE mo_cloudtypes,      ONLY: nclusters, nclusters_matus, compute_cloud_labels, aggregate_labels_matus
...
CONTAINS
  ...
  SUBROUTINE psrad_interface( &
       ...
       & flx_upsw_ct     ,flx_dnsw_ct     ,flx_uplw_ct     ,flx_dnlw_ct     ,&
    ...
    USE mo_param_switches,       ONLY: lAP3D
    !<<DN
    ...
    REAL(WP),INTENT(IN)  ::            &
         ...
         picnc(kbdim,klev),            & !< ice crystal number concentration
    ...
    REAL (wp), INTENT(OUT) ::                 &
         !   upward LW flux profile, per cloud type
         flx_uplw_ct(kbdim,klev+1,2*nclusters+nclusters_matus), &
         ! downward LW flux profile, per cloud type
         flx_dnlw_ct(kbdim,klev+1,2*nclusters+nclusters_matus), &
          !   upward SW flux profile, per cloud type
         flx_upsw_ct(kbdim,klev+1,2*nclusters+nclusters_matus), &
         ! downward SW flux profile, per cloud type
         flx_dnsw_ct(kbdim,klev+1,2*nclusters+nclusters_matus)

    ! -------------------------------------------------------------------------------------
    INTEGER  :: jk, jl, jp, jkb, nc,      & !< loop indicies
    ...
    INTEGER :: cld_labels(kbdim,klev)       !< original cloud labels
    INTEGER :: cld_labels0(kbdim,klev)      !< matus cloud labels
    INTEGER :: cld_labels_vr(kbdim,klev)    !< matus cloud labels, vertically reversed
    INTEGER :: cld_labels0_vr(kbdim,klev)   !< original cloud labels, vertically reversed
    !
...
!<<DN
    ...
    REAL(wp) :: flx_uplw_ct_vr(kbdim,klev+1,2*nclusters+nclusters_matus)
    REAL(wp) :: flx_dnlw_ct_vr(kbdim,klev+1,2*nclusters+nclusters_matus)
...
!<<DN
    ...
    CALL aggregate_labels_matus(&
            !--IN
            kproma, kbdim, klev, cld_labels0, pp_hl, &
            !--OUT
            cld_labels)

    DO jk=1,klev
       jkb = klev+1-jk
       cld_labels0_vr(1:kproma,jk) = cld_labels0(1:kproma,jkb)
       cld_labels_vr(1:kproma,jk) = cld_labels(1:kproma,jkb)
    END DO !jk
...
!<<DN
    ...
    CALL lrtm(kproma                                                          ,&
         ...
         & cld_labels0_vr  ,cld_labels_vr   ,flx_dnlw_ct_vr  ,flx_uplw_ct_vr  ,&
    ...
    CALL srtm(kproma                                                           , & 
         ...
         &  cld_labels0_vr  ,cld_labels_vr   ,flx_dnsw_ct     ,flx_upsw_ct     , &
    ...
    DO nc=1,2*nclusters+nclusters_matus
       flx_dnlw_ct(1:kproma,1:klev+1,nc) = flx_dnlw_ct_vr(1:kproma,klev+1:1:-1,nc)
       flx_uplw_ct(1:kproma,1:klev+1,nc) = flx_uplw_ct_vr(1:kproma,klev+1:1:-1,nc)
    END DO ! nc
