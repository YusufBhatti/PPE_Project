...
MODULE mo_column
  ...
  INTEGER  :: nfor_ni(3)  =  (/0,0,0/)
  INTEGER  :: nfor_qirim(3)  =  (/0,0,0/)
  INTEGER  :: nfor_birim(3)  =  (/0,0,0/)
  INTEGER  :: nfor_xl(3)  =  (/0,0,0/)
  INTEGER  :: nfor_nl(3)  =  (/0,0,0/)
  INTEGER  :: nfor_nas(3)  =  (/0,0,0/)
  INTEGER  :: nfor_ms4as(3)  =  (/0,0,0/)
  INTEGER  :: nfor_mduas(3)  =  (/0,0,0/)
...
CONTAINS
  ...
  USE mo_p3_fields,   ONLY:idt_qirim, idt_birim
  USE mo_activ,       ONLY:idt_icnc, idt_cdnc
  USE mo_ham_m7_trac, ONLY:idt_nas, idt_ms4as, idt_mduas
  ! local 
  ...
  IF (p_pe == p_io) THEN
    ...
    CALL read_initial3('nl',xt(:,:,idt_cdnc,:),'optional')
    CALL read_initial3('xi',xi,'optional')
    CALL read_initial3('ni',xt(:,:,idt_icnc,:),'optional')
    CALL read_initial3('qirim',xt(:,:,idt_qirim,:),'optional')
    CALL read_initial3('birim',xt(:,:,idt_birim,:),'optional')

    CALL read_initial3('nas',xt(:,:,idt_nas,:),'optional')
    CALL read_initial3('ms4as',xt(:,:,idt_ms4as,:),'optional')
    CALL read_initial3('mduas',xt(:,:,idt_mduas,:),'optional')
  ...
  USE mo_p3_fields,   ONLY: idt_birim, idt_qirim, nximult
  USE mo_activ,       ONLY: idt_icnc, idt_cdnc
  USE mo_ham_m7_trac, ONLY: idt_nas, idt_ms4as, idt_mduas
  EXTERNAL pres
  ...
  IF(nfor_ni(2)>0) CALL get_ext_state("ni",xt(:,:,idt_icnc,:),nfor_ni(2),nfor_ni(3))
  IF(nfor_nl(2)>0) CALL get_ext_state("nl",xt(:,:,idt_cdnc,:),nfor_nl(2),nfor_nl(3))
  IF(nfor_qirim(2)>0) CALL get_ext_state("qirim",xt(:,:,idt_qirim,:),nfor_qirim(2),nfor_qirim(3))
  IF(nfor_birim(2)>0) CALL get_ext_state("birim",xt(:,:,idt_birim,:),nfor_birim(2),nfor_birim(3))
  IF(nfor_xl(2)>0) CALL get_ext_state("xl",xl,nfor_xl(2),nfor_xl(3))

  IF(nfor_nas(2)>0) CALL get_ext_state("nas",xt(:,:,idt_nas,:),nfor_nas(2),nfor_nas(3))
  IF(nfor_ms4as(2)>0) CALL get_ext_state("ms4as",xt(:,:,idt_ms4as,:),nfor_ms4as(2),nfor_ms4as(3))
  IF(nfor_mduas(2)>0) CALL get_ext_state("mduas",xt(:,:,idt_mduas,:),nfor_mduas(2),nfor_mduas(3))
  ...
  USE mo_scan_buffer, ONLY: qte, xlte, xite, xtte
  USE mo_memory_gl,   ONLY: q, xi, xl, xt
  USE mo_p3_fields,   ONLY: idt_qirim, idt_birim, nximult
  USE mo_activ,       ONLY: idt_icnc, idt_cdnc
  USE mo_ham_m7_trac, ONLY: idt_nas, idt_ms4as, idt_mduas
  ...
  xi= MAX(xi,0.0_wp)
  xl= MAX(xl,0.0_wp)
  xt(:,:,idt_birim,:) = MAX(xt(:,:,idt_birim,:), 0.0_wp)
  xt(:,:,idt_qirim,:) = MAX(xt(:,:,idt_qirim,:), 0.0_wp)
  xt(:,:,idt_icnc,:) = MAX(xt(:,:,idt_icnc,:), 0.0_wp)
  xt(:,:,idt_cdnc,:) = MAX(xt(:,:,idt_cdnc,:), 0.0_wp)

  xt(:,:,idt_nas,:) = MAX(xt(:,:,idt_nas,:), 0.0_wp)
  xt(:,:,idt_ms4as,:) = MAX(xt(:,:,idt_ms4as,:), 0.0_wp)
  xt(:,:,idt_mduas,:) = MAX(xt(:,:,idt_mduas,:), 0.0_wp)
  ...
  IF(nfor_ni(1)>0) CALL get_ext_tend("ddt_ni",xtte(:,:,idt_icnc,:),nfor_ni(3),tendency_optn)
  IF(nfor_nl(1)>0) CALL get_ext_tend("ddt_nl",xtte(:,:,idt_cdnc,:),nfor_nl(3),tendency_optn)
  IF(nfor_qirim(1)>0) CALL get_ext_tend("ddt_qirim",xtte(:,:,idt_qirim,:),nfor_qirim(3),tendency_optn)
  IF(nfor_birim(1)>0) CALL get_ext_tend("ddt_birim",xtte(:,:,idt_birim,:),nfor_birim(3),tendency_optn)

  IF(nfor_nas(1)>0) CALL get_ext_tend("ddt_nas",xtte(:,:,idt_nas,:),nfor_nas(3),tendency_optn)
  IF(nfor_ms4as(1)>0) CALL get_ext_tend("ddt_ms4as",xtte(:,:,idt_ms4as,:),nfor_ms4as(3),tendency_optn)
  IF(nfor_mduas(1)>0) CALL get_ext_tend("ddt_mduas",xtte(:,:,idt_mduas,:),nfor_mduas(3),tendency_optn)
