...
! ----- OUTPUTS -----
  integer*4 :: infind
  real(wp), intent(out), optional :: dist
...
! ----- INPUTS -----
  real(wp), dimension(:), intent(in) :: yarr, xarr, xxarr
  real(wp), intent(in) :: tol

! ----- OUTPUTS -----
  real(wp), dimension(size(xxarr)), intent(out) :: yyarr

! ----- INTERNAL -----
  real(wp), dimension(size(xarr)) :: ysort, xsort
