...
!gf #161                   paphp1, pgeom1, loland, ptm1, pxtems, pxtte)
  ...
  USE mo_physical_constants,    ONLY: grav
  !>>DN
  USE mo_hammoz_aerocom_diags,  ONLY: lHEaci, lTraj
  USE mo_hammoz_aerocom_HEaci,  ONLY: emiso2_inst
  USE mo_hammoz_aerocom_Traj,   ONLY: emidust,emidms,emibc,emissa,emissc
  USE mo_ham_species,           ONLY: id_du, id_dms, id_bc
  !<<DN
! arguments
...
! REAL(dp), INTENT(inout) :: zxtems3d(kbdim,klev,ktrac)
  ...
  IF (lTraj)  emidust(1:kproma,krow)=0._dp
  IF (lTraj)  emidms(1:kproma,krow)=0._dp
  IF (lTraj)  emibc(1:kproma,krow)=0._dp
  IF (lTraj)  emissa(1:kproma,krow)=0._dp
  IF (lTraj)  emissc(1:kproma,krow)=0._dp
  !<<DN
...
!gf #161   IF (ibc_extra(jtrac) /= 0) CALL get_emi_field(ibc_extra(jtrac),emimod(jsec,ispec),kproma,kbdim,krow,klev,lo3d,zbc2d,zbc3d)
        ...
        IF (zfactor(jtrac) > 0._dp) THEN
          ...
          END IF
          !>>DN
          ! emidust and emibc need to be deactivated since they are 18 orders of magnitude too large for an unknown reason
          IF (lHEaci .AND. jspec == id_so2) emiso2_inst(1:kproma,krow)=emiso2_inst(1:kproma,krow)+zemidiag(1:kproma)
          !IF (lTraj .AND. jspec == id_du)   emidust(1:kproma,krow)=emidust(1:kproma,krow)+zemidiag(1:kproma)
          IF (lTraj .AND. jspec == id_dms)  emidms(1:kproma,krow)=emidms(1:kproma,krow)+zemidiag(1:kproma)
          !IF (lTraj .AND. jspec == id_bc)   emibc(1:kproma,krow)=emibc(1:kproma,krow)+zemidiag(1:kproma)
          !<<DN
          ! total mass flux
