...
!------------------------------------------------------------------------------
  ...
  !
    ...
    DO
      ...
      !------------------------------------------
      output_factor = MERGE(interval_factor, 1._dp, info%laccu .AND. info%ldivi)
...
!------------------------------------------------------------------------------
  SUBROUTINE write_var (info, stream, xzy, nland)
    ...
    INTEGER :: nmiss
    LOGICAL :: lincl                     ! RD: to turn of faulty SCM streams
...
! RD: debug turn off scm-problem streams
        lincl = stream%name /= "jsbach" .and. & 
                stream%name /= "land"   .and. &
                stream%name /= "veg"
        IF(lincl) THEN
        IF (nprocio == 0) THEN
