...
CONTAINS
  ...
  SUBROUTINE lrtm     (kproma                     ,&
       ...
       & cld_labels0,cld_labels,dflx_ct ,uflx_ct  ,&
       & tlay       ,tlev     ,tsfc     ,wkl      ,&
    ...
    INTEGER, INTENT(in) :: cld_labels0(kbdim,klev) !< original cloud labels
    INTEGER, INTENT(in) :: cld_labels(kbdim,klev)  !< matus cloud aggregate labels
    ...
    !< upward flux for cloud types [W/m2]
    REAL(wp), INTENT(OUT) :: uflx_ct(kbdim,0:klev,2*nclusters+nclusters_matus)
    !< upward flux for cloud types [W/m2]
    REAL(wp), INTENT(OUT) :: dflx_ct(kbdim,0:klev,2*nclusters+nclusters_matus)
    ...
    REAL(wp) ::                            &
         ...
         & tautct(kbdim,klev),            & !< tautot excluding each cloud type once
         & mask(kbdim,klev),              & !< mask excluding one particular cloud type
         & zlabels(kbdim,klev)              !< local copy of cloud labels
    ...
    INTEGER :: jl, jk, ig, nc, nnc ! loop indicies
...
!IBM* ASSERT(NODEPS)
    ...
    uflx_ct(1:kproma,0:klev,:) = 0.0_wp
    dflx_ct(1:kproma,0:klev,:) = 0.0_wp
      ...
      IF(.NOT. lspeedrun) THEN
         DO nc=1,2*nclusters+nclusters_matus
            ...
            ELSE IF(nc < 2*nclusters + 1) THEN
               ...
               nnc = nc-nclusters
               lclearsky = .TRUE.
            ! cloud radiative effect: aggregated (Matus & L'Ecuyer 2017)
            ELSE
            ...
            tautct(1:kproma,:) = taut(1:kproma,:,ig) + smp_tau(1:kproma,:,ig)*mask(1:kproma,:)
            ...
            uflx_ct(1:kproma,0:klev,nc) = uflx_ct(1:kproma,0:klev,nc) + zgpcu(1:kproma,0:klev) * gpt_scaling
            dflx_ct(1:kproma,0:klev,nc) = dflx_ct(1:kproma,0:klev,nc) + zgpcd(1:kproma,0:klev) * gpt_scaling
         END DO !nc
      END IF !lspeedrun
