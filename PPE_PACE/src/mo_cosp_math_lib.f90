...
! ----- INTERNAL -----  
  ...
  if (x ==int(x)) then
    ...
    data g/1.0_wp,0.5772156649015329_wp, &
           -0.6558780715202538_wp, -0.420026350340952d-1, &
           0.1665386113822915_wp,-.421977345555443d-1, &
...
! ----- INPUTS -----  
  real(wp), intent(in), dimension(:) :: f,s  
...
!  Parameters:
!
!    Input, real*8 ( kind = 8 ) FTAB(NTAB), the function values,
!    FTAB(I) = F(XTAB(I)).
!
!    Input, real*8 ( kind = 8 ) XTAB(NTAB), the abscissas at which the
...
!
!    Input, real*8 ( kind = 8 ) A, the lower limit of integration.  A should
!    be, but need not be, near one endpoint of the interval
!    (X(1), X(NTAB)).
!
!    Input, real*8 ( kind = 8 ) B, the upper limit of integration.  B should
!    be, but need not be, near one endpoint of the interval
!    (X(1), X(NTAB)).
!
!    Output, real*8 ( kind = 8 ) RESULT, the approximate value of the integral.

!CNam: replaced KR8 with _wp
  integer, intent(in) :: ntab

  real(wp), intent(in) :: a_in
  real(wp) a
  real(wp) atemp
  real(wp), intent(in) :: b_in
  real(wp) b
  real(wp) btemp
  real(wp) ca
  real(wp) cb
