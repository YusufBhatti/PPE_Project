  module mo_cosp_radar_simulator_types
...
! Updated by Roj Marchand June 2010 
  ...
  real(wp), parameter ::        &
  dmin = 0.1_wp                 ,& ! min size of discrete particle
  dmax = 10000._wp                 ! max size of discrete particle
...
! ---- hydrometeor class type -----  
  ...
  type class_param
    ...
    real(wp)  :: step_list(Re_MAX_BIN),base_list(Re_MAX_BIN)
    ...
    real(wp) :: D(nd) ! set of discrete diameters used to represent DSDs

  end type class_param
  
   
  end module mo_cosp_radar_simulator_types
