...
! $URL: http://cfmip-obs-sim.googlecode.com/svn/stable/v1.4.0/quickbeam/zeff.f90 $
  subroutine cosp_zeff(freq,D,N,nsizes,k2,tt,ice,xr,z_eff,z_ray,kr,qe,qs,rho_e)
...
! ----- INPUTS -----  
  ...
  real(wp), intent(in) :: freq,D(nsizes),N(nsizes),tt,qe(nsizes), &
    qs(nsizes), rho_e(nsizes)
  real(wp), intent(inout) :: k2
  
! ----- OUTPUTS -----
  real(wp), intent(out) :: z_eff,z_ray,kr
...
! ----- INTERNAL -----
  ...
  f				! ice fraction
  real(wp), dimension(nsizes) :: xtemp
  real(wp) :: &
  wl, &				! wavelength (m)
  ...
  integer*4 :: i, one
  real(wp) :: pi
  real(wp) :: eta_sum, eta_mie, const, z0_eff, z0_ray, k_sum, &
...
! // conversions
  D0 = d*1.E-6_wp			! m
  N0 = n*1.E12_wp			! 1/(m^3 m)
  wl = 2.99792458_wp/(freq*10._wp)	! m

  
! // dielectric constant |k^2| defaults
  if (k2 < 0) then
    k2 = 0.933_wp
    if (abs(94._wp-freq) < 3._wp) k2=0.75_wp
    if (abs(35._wp-freq) < 3._wp) k2=0.88_wp
    if (abs(13.8_wp-freq) < 3._wp) k2=0.925_wp
  endif  
...
!   :: correct refractive index for ice density if needed
    if (correct_for_rho == 1) then
      f = rho_e/rho_ice
      m0 = ((2+m0**2+2*f*(m0**2-1))/(2+m0**2+f*(1-m0**2)))**(0.5_wp)
    endif       
...
! // convert to mm^6/m^3
  z_eff = z0_eff*1.E18_wp !  10.*alog10(z0_eff*1E18)
  z_ray = z0_ray*1.E18_wp !  10.*alog10(z0_ray*1E18)
  
  end subroutine cosp_zeff
