...
USE mo_activ,             ONLY: icnc_instantan
  ...
  USE mo_cover,             ONLY: cover, cover2
  ...
  !ham_ps:  via call_submodels: xtdriver1, xtdriver2, xtdiagn
  !
...
!>>SF
  ...
  USE mo_cloud_micro_p3, ONLY: cloud_micro_interface_p3
  USE mo_remos,          ONLY: drieff
  USE mo_p3_fields,      ONLY: idt_qirim, idt_birim, iprog, lspeedrun
  USE mo_cloudtypes,     ONLY: cloudtype_diagnostic, nclusters, nclusters_matus, &
                               csemter_ct, cstrsol_ct
  USE mo_histogram,      ONLY: histogram_diagnostic
!<<SF
...
!<<SF
  ...
  INTEGER :: jlev, jl, kfdia, kidia, ktdia, jk, nexp, nc
...
REAL(dp):: zqirim(ldc%nproma,nlev)
REAL(dp):: zbirim(ldc%nproma,nlev)

! Cloud radiative effect of different cloud types. Not in mo_memory_g3b because
! there would be too much data for each cloud type  
  REAL(dp) :: emter_ct(ldc%nproma,nlevp1,2*nclusters+nclusters_matus)
  REAL(dp) :: trsol_ct(ldc%nproma,nlevp1,2*nclusters+nclusters_matus)
...
!           See Seinfeld/Pandis p.808
  ...
  zqirim(1:nproma,1:nlev) = xtm1(1:nproma,1:nlev,idt_qirim,krow)
  zbirim(1:nproma,1:nlev) = xtm1(1:nproma,1:nlev,idt_birim,krow)
...
4106 END DO
  ...
  IF(locond) THEN
    ...
    ! CALL cover2(nproma, nbdim, ktdia, nlev, &
    !             tm1(:,:,krow), apm1, aphm1, qm1(:,:,krow), xim1(:,:,krow), &
    !             aclc(:,:,krow))
...
!>>SF
           & ,geom1                  ,co2m1(:,:,krow)         ,acdnc(:,:,krow)        ,zicnc(:,:)               &
           & ,aclc(:,:,krow)                                                                                    &
!<<SF
           ...
           & ,trsol(:,:,krow)        ,emter_ct(:,:,:)         ,trsol_ct(:,:,:)        ,ao3(:,:,krow)            )

      ! update stored value if radiation is called
      DO nc=1,2*nclusters+nclusters_matus
         csemter_ct(nc)%v(1:nproma,:,krow) = emter_ct(1:nproma,:,nc)
         cstrsol_ct(nc)%v(1:nproma,:,krow) = trsol_ct(1:nproma,:,nc)
      END DO !nc

      IF (ltimer) CALL timer_stop(timer_radiation)
    END IF

    ! read back if radiation is not called
    DO nc=1,2*nclusters+nclusters_matus
       emter_ct(1:nproma,:,nc) = csemter_ct(nc)%v(1:nproma,:,krow)
       trsol_ct(1:nproma,:,nc) = cstrsol_ct(nc)%v(1:nproma,:,krow)
    END DO !nc
  ...
  ELSE
    ...
    DO nc=1,2*nclusters+nclusters_matus
       emter_ct(1:nproma,:,nc) = 0._dp
       trsol_ct(1:nproma,:,nc) = 1._dp
    END DO !nc
    ...
    ELSE 
       ...
       IF(iprog==3) THEN
          CALL cloud_micro_interface(nproma, nbdim, nlev, nlevp1, ntrac, ktdia, krow,       & ! in
                                     ...
                                     ssfl(:), rsfl(:))                                        ! out
       ELSE
          CALL cloud_micro_interface_p3(nproma, nbdim, nlev, nlevp1, ntrac, ktdia, krow,       & ! in
                                     invb,                                                  & ! in
                                     aphm1,           apm1,              app1,              & ! in
                                     qm1(:,:,krow),   tm1(:,:,krow),     ztvm1,             & ! in
                                     xlm1(:,:,krow),    xim1(:,:,krow),                     & ! in
       ...
       IF(.NOT. lspeedrun) THEN
          CALL histogram_diagnostic(nproma, nbdim, nlev, nlevp1, ntrac, ktdia, krow,          & ! in
                                    aphm1, apm1, ztvm1, geom1, aclc(:,:,krow), tm1(:,:,krow), & ! in
                                    qm1(:,:,krow), xlm1(:,:,krow), xim1(:,:,krow),            & ! in
                                    xtm1(:,:,:,krow), ssfl(:), rsfl(:))                         ! in

          CALL cloudtype_diagnostic(nproma, nbdim, nlev, nlevp1, ntrac, ktdia, krow,         & ! in
                                    aphm1, apm1, geom1, aclc(:,:,krow), tm1(:,:,krow),       & ! in
                                    qm1(:,:,krow), xlm1(:,:,krow), xim1(:,:,krow),           & ! in
                                    xtm1(:,:,:,krow), ssfl(:), rsfl(:), ssfc(:), rsfc(:),    & ! in
                                    ictop, tkem1(:,:,krow),                                  & ! in
                                    vervel(:,:,krow), ztvm1, emter(:,:,krow),                & ! in
                                    emtef0(:,:,krow), trsol(:,:,krow), trsof0(:,:,krow),     & ! in
                                    emter_ct, trsol_ct,                                      & ! in 
                                    zi0(:), rdayl_x(:,krow))                                   ! in
       END IF !lspeedrun
...
!>>KH compatibility with v1.4 of cosp
