  subroutine cosp_radar_simulator_init(freq,k2,use_gas_abs,do_ray,undef, &
                  nhclass, &
...
! ----- INTERNAL -----  
  ...
  real(wp)  :: delt, deltp
    ...
    hp%base_list(1)=0;
    do j=1,Re_MAX_BIN
        hp%step_list(j)=0.1_wp+0.1_wp*((j-1)**1.5_wp);
    ...
    !
    do i=1,cnt_ice
        hp%mt_tti(i)=(i-1)*5-90 + 273.15_wp
    enddo
    
    do i=1,cnt_liq
        hp%mt_ttl(i)=(i-1)*5-60 + 273.15_wp
    enddo 
  ...
  end subroutine cosp_radar_simulator_init
