...
!<<SF
       ...
       & ,sw_net            ,lw_net_ct       ,sw_net_ct        ,ozone         )
    ...
    USE mo_hammoz_aerocom_HEaci, ONLY: rsut_tmp, rlut_inst, rsutcs_tmp, rlutcs_inst, &
         rsutnoa_tmp, rsutcsnoa_tmp, rsdt_tmp, rlutnoa_inst, rlutcsnoa_inst
    !<<DN
...
!<<SF
    ...
    REAL(wp), INTENT(OUT) ::      &
         ! Cloud type specific net downward longwave at all levels
         lw_net_ct(kbdim,klevp1,2*nclusters+nclusters_matus), &
         ! Cloud type specific net downward longwave at all levels
         sw_net_ct(kbdim,klevp1,2*nclusters+nclusters_matus)

    REAL(wp), INTENT(INOUT) ::    & ! Avoid leaving kproma+1:kbdim undefined
         ozone(kbdim,klev)          !< Ozone 

    INTEGER  :: jk, jl, idx(kbdim), iaero_call, number_rad_call, i_rad_call, id_tracer, nc
    ...
    ! cloud type flux profiles
    REAL (wp) ::                              &
         flx_uplw_ct(kbdim,klevp1,2*nclusters+nclusters_matus), & !   upward LW flux profile
         flx_dnlw_ct(kbdim,klevp1,2*nclusters+nclusters_matus), & ! downward LW flux profile
         flx_upsw_ct(kbdim,klevp1,2*nclusters+nclusters_matus), & !   upward SW flux profile
         flx_dnsw_ct(kbdim,klevp1,2*nclusters+nclusters_matus)    ! downward SW flux profile
...
!------------------------------------------------------------------------
    ...
    DO i_rad_call = 1,number_rad_call
      ...
      DO nc=1,2*nclusters+nclusters_matus
         lw_net_ct(1:kproma,1:klevp1,nc) = flx_dnlw_ct(1:kproma,1:klevp1,nc)     &
                                         - flx_uplw_ct(1:kproma,1:klevp1,nc)       
         sw_net_ct(1:kproma,1:klevp1,nc) = (flx_dnsw_ct(1:kproma,1:klevp1,nc)    &
                                         - flx_upsw_ct(1:kproma,1:klevp1,nc))       &
              &                          * SPREAD(flux_factor(1:kproma),2,klevp1)
      END DO ! nc
    ...
    IF (laerocom_diag.AND.lHEaci) THEN
       ...
       ELSE
          ...
          rsdt_tmp(1:kproma,krow)= flx_dnsw(1:kproma,1)*flux_factor(1:kproma) !TOA
       ENDIF
    ENDIF
    !<<DN
