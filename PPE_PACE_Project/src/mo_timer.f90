...
CONTAINS
  ...
  ! Transpose timer
    ...
    timer_s2l               = new_timer('spectral_to_legendre')    
    timer_l2s               = new_timer('legendre_to_spectral')
    timer_f2l               = new_timer('fourier_to_legendre')
    timer_l2f               = new_timer('legendre_to_fourier')
    timer_g2f               = new_timer('gridpoint_to_fourier')
    timer_f2g               = new_timer('fourier_to_gridpoint')
    ...
    timer_gather2           = new_timer('gather_2d')
    timer_gather3           = new_timer('gather_3d')    

    timer_time_set          = new_timer('stepon:time_manager_set')    
    timer_time_reset        = new_timer('stepon:time_manager_reset')    
    timer_trigfiles         = new_timer('stepon:trigger_files')    
    timer_bcond             = new_timer('stepon:boundary_conditions')    
    timer_subm              = new_timer('stepon:sub_models')    
    ...
    timer_inv_legendre      = new_timer('stepon:inverse_legendre')    
    timer_prerad            = new_timer('stepon:prerad')    
    timer_output            = new_timer('stepon:output_IO')
    timer_restart           = new_timer('stepon:restart_IO')

    timer_couple_get        = new_timer('stepon:couple_get') 
    timer_couple_put        = new_timer('stepon:couple_put') 
    timer_couple_end        = new_timer('stepon:couple_end') 
    timer_hd                = new_timer('stepon:hydrology_model') 
