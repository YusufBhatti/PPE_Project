...
MODULE mo_time_control
  ...
  LOGICAL              :: lresume     = .TRUE.  ! .TRUE. during rerun step
  LOGICAL              :: lresume_cmd = .FALSE.  ! .TRUE. during rerun step
  ...
  INTEGER, PARAMETER     :: NO_PUTDATA = 60  ! number of independent io events
  ...
  TYPE(io_time_event),SAVE ::    putrerun   &! rerun write-up interval and stop
       = &
       io_time_event(1,TIME_INC_MONTHS,TRIG_LAST,0)

  LOGICAL               ::  l_putcheckpoint  = .FALSE.
  TYPE(time_event),SAVE :: ev_putcheckpoint
  TYPE(io_time_event),SAVE ::    putcheckpoint   &! rerun write-up interval
       = &
...
CONTAINS
  ...
  SUBROUTINE print_events               !***** called in CONTROL ***********
    ...
    IF (ldebugev) THEN
      ...
      WRITE(m_text,*) 'PUTCHECKPOINT= ',putcheckpoint, ','; CALL message('',m_text)
...
!!    INTEGER           :: days, seconds
  ...
  SUBROUTINE init_events     !********* called in INITIALIZE *******************
    ...
    CALL echam_ev_init(ev_putcheckpoint, putcheckpoint, 'checkpoint interval', EV_TLEV_NEXT)
        ...
        DO
          ...
          IF (PRESENT(eval_date)) THEN
            ...
            CASE(EV_TLEV_PRES)          ! check with current date
              IF (.NOT. my_date < current_date) EXIT
              
            CASE(EV_TLEV_NEXT)          ! check with next date
              IF (.NOT. my_date < next_date) EXIT
              
            END SELECT
          ELSE
            IF (my_date > resume_date) EXIT

          END IF
          ! Trigger event if possible
          triggered = event_state(event, my_date)
        END DO
...
!ik_bugfix_R1.02a
    ...
    l_putcheckpoint = event_state(ev_putcheckpoint, next_date)
...
!      l2nd_day = next_date < my_day     ! new version
    ...
    IF (l_putrerun) THEN
      IF (no_cycles >= 1) THEN
        ...
        l_putcheckpoint = .FALSE.         ! do not need checkpoint in case of putrerun
      ELSE
    ...
    IF (ldebugev) THEN
      ...
      IF (l_putcheckpoint)  CALL echam_ev_print(ev_putcheckpoint)
      DO isj = 1,nsub
...
!<<SF
    ...
    ELSE IF (nlevel == 47) THEN
      ...
      IF      (truncation == 31) THEN; set_delta_time =  900.0_dp ! unsupported!
    ...
    ELSE IF (nlevel == 95) THEN  
      ...
      ELSE IF (truncation ==127) THEN; set_delta_time =  200.0_dp
      ELSE IF (truncation ==255) THEN; set_delta_time =  90.0_dp
      ELSE
        CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE
  ...
  SUBROUTINE get_orbit_times(lrad_date, lyr_perp, nmonth, yr_perp            &
    ...
    ELSE
      ...
      orbit_date = MOD(zscr/get_year_len(),1.0_dp)*2.0_dp*pi
    END IF
...
!--jsr-mag
  ...
  FUNCTION get_clock (date) RESULT (zclock)  !**********************************
    ...
    zclock = REAL(jsec, dp)/day_len()*2*pi
