...
!>>SF
#ifdef HAMMOZ
  INTEGER  :: ierr, ibc_cvcbot, ibc_wcape, ibc_tconv
#endif
!<<SF
...
#endif
#ifdef HAMMOZ  
    REAL(wp):: zqtmst,zxtte(kbdim,klev,ktrac)
#endif
    !
    ...
    IF (lmoz) THEN
       DO jt=1,ktrac
          IF (trlist%ti(jt)%nphase==GAS) THEN
             ! limit tracer tendencies
             pxtte(1:kproma,:,jt) = zxtte(1:kproma,:,jt) &
                                  + MAX(pxtte(1:kproma,:,jt)-zxtte(1:kproma,:,jt),-zxtp1(1:kproma,:,jt)*zqtmst)
          END IF
       END DO
    END IF
    !>>UP ham timers
...
!>>SF
#ifdef HAMMOZ
    REAL(wp) :: zxtbound(kbdim,ktrac) !SF boundary condition for xt_conv_massfix
    REAL(wp) :: zcvcbot(kbdim), zwcape(kbdim)
#endif
!<<SF
    !
...
! UP comment: to me it seems like this does not contribute to HAM directly, so I don't include it in timer_ham_totsum.
    IF (lclmi_progn) THEN
        DO jl=1,kproma
           ...
           zwcape(jl)=0._wp
           IF(ldcum(jl)) THEN
              IF (zcape(jl).GT.0._wp) THEN
                 zcvcbot(jl)=REAL(kcbot(jl),wp)
                 zwcape(jl)=2._wp*SQRT(zcape(jl))
              ENDIF
           ENDIF
        ENDDO
