...
!<<DN #475 (min CDNC)
  !>>DN AeroCom
  REAL(dp) :: zraut(kbdim,klev),zracl(kbdim,klev),zsacl_2d(kbdim,klev)
  !<<DN AeroCom
...
!                   zmratepr(:,jk), zrpr(:), zrprn(:,jk) )
                   zmratepr(:,jk), zrpr(:), zrprn(:,jk), zracl(:,jk), zraut(:,jk) )
           !<<DN AeroCom
        ELSE
            ...
            zrpr(1:kproma)        = 0._dp
            !>>DN AeroCom
            zracl(1:kproma,jk)=0._dp
            zraut(1:kproma,jk)=0._dp
            !<<DN AeroCom
        ENDIF
...
!            Effective radius of ice crystals assuming plates (Lohmann, ACPD, 2007)
        ...
        !>>DN AeroCom
        zsacl_2d(1:kproma,jk) = zsacl(1:kproma)
        !<<DN AeroCom
...
!>>DN AeroCom
  ...
  CALL cloud_subm_3( &
       !-- IN   
       kproma, kbdim, klev, krow, itop(:,:), ll_liqcl(:,:), &
       zclcov(:), zcdnc(:,:), zrho(:,:), paclc(:,:), reffl(:,:,krow), prsfl(:), pssfl(:), &
       zraut(:,:), zracl(:,:), zdpg(:,:), zsacl_2d(:,:), zfrain(:,:), zfevapr(:,:), zclcpre_2d(:,:))
!<<DN AeroCom
...
!>>DN AeroCom
USE mo_hammoz_aerocom_diags, ONLY: lMMPPE
USE mo_param_switches,       ONLY: ac_scale_autoconversion, ac_scale_accretion, ac_scale_KK_LWP_exponent
!<<DN AeroCom
!YAB
USE mo_hammoz_perturbations, ONLY: lo_hammoz_perturbations, KK_exponent, KK_LWP_exponent
!YAB
  INTEGER, INTENT(in) :: kbdim, kproma
  ...
  !>>DN AeroCom
  REAL(dp), INTENT(out) :: pracl(kbdim),praut(kbdim)
  !<<DN AeroCom
  !local vars:
...
!          Autoconversion rate from Khairoutdinov and Kogan, 2000
     ...
     IF (.NOT.lMMPPE) THEN
        ...
        exm1_1=2.47_dp-1.0_dp
        exp_1=-1._dp/exm1_1
     ELSE
        ztmp1(1:kproma) = ccraut*1350._dp*(1.e-6_dp*pcdnc(1:kproma))**ac_scale_autoconversion
        exm1_1=ac_scale_KK_LWP_exponent-1.0_dp
        exp_1=-1._dp/exm1_1
     END IF
     !<<DN AeroCom
...
!--- zrac2 is formed by accretion with newly formed rain inside the grid box
     ...
     IF (lMMPPE) zrac1(1:kproma) = ac_scale_accretion*zrac1(1:kproma) !DN AeroCom
     ...
     IF (lMMPPE) zrac2(1:kproma) = ac_scale_accretion*zrac2(1:kproma) !DN AeroCom
