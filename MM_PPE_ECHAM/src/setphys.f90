...
SUBROUTINE setphys
  ...
  USE mo_param_switches, ONLY: lphys, lrad, lvdiff, lcond, lsurf,  &
                               ...
                               nactivpdf, & !ZK
                               ac_scale_activation, ac_scale_autoconversion,& !DN
                               ac_scale_accretion, ac_scale_KK_LWP_exponent !DN
...
!<<DN
  ...
  USE mo_submodel,       ONLY: lccnclim, print_value, lham, lhammoz !++mgs 
  USE mo_hammoz_perturbations, ONLY: init_hammoz_perturbations
...
!<<gf
    ...
    IF (lham .OR. lhammoz) THEN
      CALL init_hammoz_perturbations
    ENDIF
