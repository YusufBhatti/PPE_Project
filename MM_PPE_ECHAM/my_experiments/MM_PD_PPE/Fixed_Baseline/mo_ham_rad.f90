...
!<<NAJS
    USE mo_ham_streams,  ONLY: tau_mode, abs_mode, omega_mode, sigma_mode, asym_mode,    &
                               tau_comp, abs_comp, tau_2d,     abs_2d,     omega_2d, ang, ai, &
!>>PS: optical properties for dry aerosol
...
! YAB 2d tau mode
          ...
          !--- total single scattering albedo, derived from total tau/abs:

          ll1(1:kproma) = (tau_2d(jwv)%ptr(1:kproma,krow) > zeps)
          omega_2d(jwv)%ptr(1:kproma,krow) = MERGE(                                    &
               1.0_dp - abs_2d(jwv)%ptr(1:kproma,krow)/                               &
               MERGE(tau_2d(jwv)%ptr(1:kproma,krow), 1._dp, ll1(1:kproma)),            &
               0._dp, ll1(1:kproma))
