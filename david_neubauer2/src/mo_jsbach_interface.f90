...
!>>DN cvs fix
    REAL(dp), OPTIONAL, INTENT(out)   :: snow_fraction(kdim)       !! snow fraction over land
!<<DN
...
!>>DN cvs fix
    REAL(dp), DIMENSION(kland) ::   zsnow_fraction                     !! Packed snow fraction over land
!<<DN
...
!>>DN cvs fix
       IF (PRESENT(snow_fraction)) snow_fraction                   = 0.0_dp
!<<DN
...
!>>DN cvs fix
    CALL average_tiles(theLand%Soil%snow_fract(kidx0:kidx1,:), theLand%Surface%is_present(kidx0:kidx1,:), &
                       theLand%Surface%cover_fract(kidx0:kidx1,:), zsnow_fraction(:))
!<<DN
...
!>>DN cvs fix
   IF (PRESENT(snow_fraction)) snow_fraction                   = UNPACK(zsnow_fraction,mask,0.0_dp)
!<<DN cvs fix
