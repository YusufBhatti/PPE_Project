  subroutine cosp_radar_simulator( &
    hp, &
    nprof,ngate, &
    undef, &
  ...
  use mo_cosp_optics_lib
  use mo_cosp_radar_simulator_types
...
! ----- INPUTS -----  
  ...
  real(wp) undef
  real(wp), dimension(nprof,ngate), intent(in) :: &
    hgt_matrix, p_matrix,t_matrix,rh_matrix

  real(wp), dimension(hp%nhclass,nprof,ngate), intent(in) :: hm_matrix
  real(wp), dimension(hp%nhclass,nprof,ngate), intent(inout) :: re_matrix
  real(wp), dimension(hp%nhclass,nprof,ngate), intent(in)    :: Np_matrix

! ----- OUTPUTS -----
  real(wp), dimension(nprof,ngate), intent(out) :: Ze_non,Ze_ray, &
       g_to_vol,dBZe,a_to_vol

! ----- OPTIONAL -----
  real(wp), optional, dimension(nprof,ngate) :: &
...
!  real(wp), dimension(nprof,ngate) :: kr_matrix 
...
! ----- INTERNAL -----
  ...
  cosp_gases      ! function: 2-way gas atten (dB/km) !CNam: cosp_gases
  ...
  real(wp), parameter :: xx = -1.0_wp !CNam: KR8
  real(wp),  dimension(:), allocatable :: xxa
  real(wp) :: kr, ze, zr, pi, scale_factor, tc, Re, ld, tmp1, ze2, kr2, apm, bpm
  real(wp) :: half_a_atten_current,half_a_atten_above
  real(wp) :: half_g_atten_current,half_g_atten_above
  integer*4 :: tp, i, j, k, pr, itt, iff

  real(wp)    step,base, Np
...
!     :: if there is hydrometeor in the volume
      if (hydro) then
        ...
        rho_a = (p_matrix(pr,k)*100._wp)/(287.0_wp*(t_kelvin))
!       :: loop over hydrometeor type
        do tp=1,hp%nhclass
          ...
          if (re_matrix(tp,pr,k).eq.0) then
            call cosp_calc_Re(hm_matrix(tp,pr,k),Np_matrix(tp,pr,k),rho_a, &
...
!                    'in Look-Up Table (LUT).  Will calculate. '
          ...
          if( (.not. hp%Z_scale_flag(tp,itt,iRe_type)) .or. DO_LUT_TEST)  then
            ...
            call cosp_dsd(hm_matrix(tp,pr,k),re_matrix(tp,pr,k),Np_matrix(tp,pr,k), &
            ...
            if (phase == 1) then
              if (hp%rho(tp) < 0) then
                ...
                Deq = ( ( 6/pi*hp%apm(tp)/917 ) ** (1.0_wp/3.0_wp) ) * ( (Di*1E-6) ** (hp%bpm(tp)/3.0_wp) )  * 1E6
                ...
                Deq=Di * ((hp%rho(tp)/917)**(1.0_wp/3.0_wp))
            ...
            call cosp_zeff(hp%freq,Deq,Ni,ns,hp%k2,t_kelvin,phase,hp%do_ray, &
...
!     :: volume is hydrometeor-free	
      ...
      if (g_to_vol_in_present) then
        ...
        if ( (hp%use_gas_abs == 1) .or. ((hp%use_gas_abs == 2) .and. (pr == 1)) ) then
          g_vol(pr,k) = cosp_gases(p_matrix(pr,k),t_kelvin,rh_matrix(pr,k),hp%freq) !CNam: cosp_gases
          if (d_gate==1) then
            if (k>1) then
              ! add to previous value to half of above layer + half of current layer
  ...
  end subroutine cosp_radar_simulator
