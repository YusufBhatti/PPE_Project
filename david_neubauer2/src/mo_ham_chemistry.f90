...
!                                   d_prod_ms4cs
  ...
  USE mo_ham_m7_processing,  ONLY: qprod_ms4cd
!<<DN: aerosol processing
...
!>>DN: aerosol processing [update in-cloud and interstitial mixing ratios]
...
!Note: this takes care of offline oxidants as well as online (HAMMOZ) fields!
  ...
  SELECT CASE(nham_subm)
      ...
      CASE(HAM_M7)
!>>DN: aerosol processing [use in-cloud sulfate when aerosol processing is activated]
!         zso4(1:kproma,:)=pxtp1(1:kproma,:,idt_ms4as)+ pxtp1(1:kproma,:,idt_ms4cs)
         IF (laeroproc) THEN
            zso4(1:kproma,:)=pxtp1(1:kproma,:,idt_cd(id_so4))
         ELSE
            zso4(1:kproma,:)=pxtp1(1:kproma,:,idt_ms4as)+ pxtp1(1:kproma,:,idt_ms4cs)
         END IF
!<<DN
...
!gf           zdso4(jl,jk)=zdso2tot
  ...
  DO jk=ktop,klev
     DO jl=1,kproma
        SELECT CASE(nham_subm)
            CASE(HAM_M7)
               ...
               IF (.NOT. laeroproc) THEN !>>DN: standard, no aerosol processing
    ...
    !gf see #103           pxtp1c(jl,jk,idt_ncs)=pxtp1c(jl,jk,idt_ncs)+(zdso4(jl,jk)/zso4massc)
            ...
            ELSE !DN: laeroproc=.true., aerosol processing
               pxtp1c(jl,jk,idt_cd(id_so4)) = pxtp1(jl,jk,idt_cd(id_so4)) + paclc(jl,jk)*zdso4(jl,jk)
               qprod_ms4cd(jl,jk,krow) = qprod_ms4cd(jl,jk,krow)                                 &
                                       + prhop1(jl,jk)*paclc(jl,jk)*zdso4(jl,jk)*delta_time/ztmst
            ENDIF !<<DN: end laeroproc choice
...
!>>DN: aerosol processing
  IF(laeroproc)THEN
     ...
     DO jk=1, klev
        DO jl=ktop, kproma
            !do not weigh again with cloud fraction
           zxtte(jl,jk,jtrac) = &
                (pxtp1c(jl,jk,jtrac)-pxtm1(jl,jk,jtrac))*zqtmst - pxtte(jl,jk,jtrac)
           ...
           pxtp1(jl,jk,jtrac)  = pxtm1(jl,jk,jtrac) + pxtte(jl,jk,jtrac) * ztmst
           pxtp1c(jl,jk,jtrac) = pxtp1(jl,jk,jtrac)
           pxtp10(jl,jk,jtrac) = pxtp1(jl,jk,jtrac)
        END DO
     END DO
  END IF
!<<DN: aerosol processing
...
!                                             pdpg(1:kproma,jk)*delta_time
            IF (laeroproc) &
                 d_prod_ms4cd(1:kproma,krow)=d_prod_ms4cd(1:kproma,krow) + &
                                             zxtte(1:kproma,jk,idt_cd(id_so4))*pdpg(1:kproma,jk)*delta_time
            IF (laeroproc) THEN
               d_prod_so4_liq(1:kproma,jk,krow)=d_prod_so4_liq(1:kproma,jk,krow) + &
                                               zxtte(1:kproma,jk,idt_cd(id_so4))*pdpg(1:kproma,jk)*delta_time
            ELSE
