...
!>>DN: aerosol processing
  USE mo_physical_constants,ONLY: rhoh2o
  USE mo_math_constants,    ONLY: pi
  USE mo_ham_m7_processing,ONLY: pnprecip
  USE mo_ham,              ONLY: laeroproc
!<<DN: aerosol processing
...
!>>DN: aerosol processing
  IF(laeroproc)THEN
     !precip mass and number
     zmprecip(1:kproma,:) = zfprec(1:kproma,:)/(pdpg(1:kproma,:)/time_step_len)
     !estimate number from average raindrop size of 100 mum
     pnprecip(1:kproma,:,krow) = zmprecip(1:kproma,:)                            &
          / (4._dp/3._dp*pi*rhoh2o*(100.e-6_dp)**3._dp)
  END IF
!<<DN: aerosol processing
...
!  USE mo_tracdef,       ONLY: ln, ntrac, trlist, AEROSOLMASS, AEROSOLNUMBER, GAS
  ...
  USE mo_ham,           ONLY: laeroproc, aerocomp
  USE mo_ham_m7_processing, ONLY: pnprecip, revap_vol, sigma_cd, dreevap, dresubl
  USE mo_ham_species,   ONLY: id_ss, id_du, id_so4
  USE mo_ham_m7ctl,     ONLY: crdiv, icoas, iaccs, iaits
  USE mo_ham_m7_trac,   ONLY: idt_ncs,idt_nks, idt_nas
  USE mo_math_constants,     ONLY: pi
  USE mo_time_control,  ONLY: time_step_len
!<<DN: aerosol processing
  USE mo_species,       ONLY: speclist
...
!>>DN: aerosol processing
  REAL(dp) :: rdry_revap(kbdim,klev),zram2cmr_cd, zeps
  INTEGER :: jtrac(kbdim,klev), jtmass(kbdim,klev),jtnum(kbdim,klev), jl, jk, kt
  LOGICAL(dp) :: lnumupdated(kbdim,klev)
  LOGICAL :: lincd,linic,lincloud,ltransfer
  REAL(dp), POINTER :: dreevap_p(:,:)
  REAL(dp), POINTER :: dresubl_p(:,:)

  IF(laeroproc)revap_vol(1:kproma,:,krow)=0._dp
  ltransfer=.TRUE.
  zeps=EPSILON(1.0_dp)
  lincd=.FALSE.
  linic=.FALSE.
...
!SFNote the above to refactor. probably should use zaclc also....
  ...
  !--- tracer loop
!>>DN: aerosol processing
  !update grid mean concentrations before re-evaporation loop
  IF(laeroproc .AND. lstrat)THEN
     DO jt=1, ntrac
        IF (trlist%ti(jt)%nwetdep == 0) CYCLE    ! do nothing if tracer doesn't get wet deposited
        lincd = trlist%ti(jt)%nphase == IN_CLOUD_LIQUID
        linic = trlist%ti(jt)%nphase == IN_CLOUD_ICE
        lincloud = lincd .OR. linic
        IF (trlist%ti(jt)%nphase == AEROSOLMASS .OR. trlist%ti(jt)%nphase == AEROSOLNUMBER .OR.&
             lincloud) THEN
           !Don't weight in-cloud modes species with cloud fraction
           DO jl=1, kproma
              DO jk=ktop,klev 
                 IF (.NOT.lincloud) THEN
                    pxtp1c(jl,jk,jt)=pxtp1c(jl,jk,jt)*zaclc(jl,jk)
                    pxtp10(jl,jk,jt)=pxtp10(jl,jk,jt)*(1._dp-zaclc(jl,jk))
                 ELSE
                    pxtp10(jl,jk,jt)=0._dp
                 END IF
              END DO
           END DO
        END IF
...
!! go backwards in order to have in-cloud modes before cs-mode 
!  DO jt=1,ntrac
!Important note: the transfer from the in-cloud aerosol tracers to the M7 aerosol tracers is coded
!                in a way now that it only works as long as the in-cloud aerosol tracers come before the 
!                M7 aerosol tracers
  DO jt=ntrac,1,-1
     lincd = trlist%ti(jt)%nphase == IN_CLOUD_LIQUID
     linic = trlist%ti(jt)%nphase == IN_CLOUD_ICE
...
!>>DN: aerosol processing
        IF ((trlist%ti(jt)%nphase == AEROSOLMASS .OR. trlist%ti(jt)%nphase == AEROSOLNUMBER) &
             .AND. ltransfer .AND. lstrat .AND. laeroproc)THEN
           ltransfer=.FALSE.
           lnumupdated(1:kproma,:) = .FALSE.
           zram2cmr_cd = 1._dp / ( EXP(1.5_dp*(LOG(1.59_dp))**2) )
            !size of released aerosol particle
           WHERE(pnprecip(1:kproma,:,krow)*zrevap(1:kproma,:)>zeps) 
              rdry_revap(1:kproma,:)=(0.75_dp/pi*revap_vol(1:kproma,:,krow) &
                   /(pnprecip(1:kproma,:,krow)*zrevap(1:kproma,:)))
              WHERE(rdry_revap(1:kproma,:)>0._dp)
                 rdry_revap(1:kproma,:)=rdry_revap(1:kproma,:)**(1._dp/3._dp)*zram2cmr_cd
              ELSEWHERE
                 rdry_revap(1:kproma,:)=0._dp
                 jtmass(1:kproma,:)=icoas
                 jtnum(1:kproma,:)=idt_ncs
              END WHERE
           ELSEWHERE
              rdry_revap(1:kproma,:)=0._dp
              jtmass(1:kproma,:)=icoas
           ...
           WHERE(rdry_revap(1:kproma,:)>0.5E-6_dp)
              rdry_revap(1:kproma,:)=rdry_revap(1:kproma,:)/EXP(1.5_dp*(LOG(sigma_cd))**2)/zram2cmr_cd
           END WHERE
           
           DO kt=1,ntrac !transfer of mass and number from in-cloud modes to aerosol modes
              IF (trlist%ti(kt)%nphase == IN_CLOUD_LIQUID.OR.&
                   trlist%ti(kt)%nphase == IN_CLOUD_ICE ) THEN
                 !put into aerosol modes, not in-cloud mode
                 ispec = trlist%ti(kt)%spid
                 
                 !>>DN [select mode for re-evaporation]                  
                 WHERE( rdry_revap(1:kproma,:) >= crdiv(iaits)*0.01_dp .AND. &
                      rdry_revap(1:kproma,:) < crdiv(iaccs)*0.01_dp .AND. &
                      (ispec/=id_du.AND.ispec/=id_ss) )
                    jtmass(1:kproma,:)=iaits
                    jtnum(1:kproma,:)=idt_nks                   
                 ELSEWHERE ( (rdry_revap(1:kproma,:) >= crdiv(iaccs)*0.01_dp .AND. &
                      rdry_revap(1:kproma,:) < crdiv(icoas)*0.01_dp) .OR.&
                      ((ispec==id_du.OR.ispec==id_ss) .AND.&
                      rdry_revap(1:kproma,:) < crdiv(icoas)*0.01_dp))
                    jtmass(1:kproma,:)=iaccs
                    jtnum(1:kproma,:)=idt_nas                  
                 ELSEWHERE ( rdry_revap(1:kproma,:) > crdiv(icoas)*0.01_dp)
                    jtmass(1:kproma,:)=icoas
                    jtnum(1:kproma,:)=idt_ncs                
                 ELSEWHERE
                    jtmass(1:kproma,:)=icoas
                 ...
                 dreevap_p     => dreevap(ispec)%ptr                                       
                 dresubl_p     => dresubl(ispec)%ptr                                       
                 DO jk=ktop,klev
                    DO jl=1,kproma
                       jtrac(jl,jk) = aerocomp(speclist(ispec)%iaerocomp(jtmass(jl,jk)))%idt
                       
                       !diagnostics
                       IF (trlist%ti(kt)%nphase == IN_CLOUD_LIQUID) THEN
                          dreevap_p(jl,krow) = dreevap_p(jl,krow) + &
                               pxtp10(jl,jk,kt)/time_step_len*pdpg(jl,jk)*delta_time
                       ELSE                          
                          dresubl_p(jl,krow) = dresubl_p(jl,krow) + &
                               pxtp10(jl,jk,kt)/time_step_len*pdpg(jl,jk)*delta_time
                       END IF

                       !gain of interstitial mass
                       pxtp10(jl,jk,jtrac(jl,jk))=pxtp10(jl,jk,jtrac(jl,jk))+&
                            pxtp10(jl,jk,kt)
                       !loss of in-cloud mass
                       pxtp10(jl,jk,kt)=0._dp    
                                      
                       !number: do only once
                       IF(.NOT. lnumupdated(jl,jk))THEN
                          pxtp10(jl,jk,jtnum(jl,jk)) = pxtp10(jl,jk,jtnum(jl,jk))+&
                               pnprecip(jl,jk,krow)*zrevap(jl,jk)
                          lnumupdated(jl,jk) = .TRUE.
                       END IF
...
!>>DN: aerosol processing
!                             jscavICphase, jscavBCphase)
                             jscavICphase, jscavBCphase, lstrat)
!<<DN: aerosol processing
...
!!$  IF(laeroproc .AND. lstrat)THEN
!!$     DO jt=1,ntrac
!!$        IF (trlist%ti(jt)%nphase == IN_CLOUD_LIQUID .OR. &
!!$             trlist%ti(jt)%nphase == IN_CLOUD_ICE ) THEN
!!$           ! reduce rounding error in tendencies calculation that lead to negative tracer values
!!$           WHERE(pxtm1(1:kproma,:,jt)>=0._dp)
!!$              pxtte(1:kproma,:,jt)=&
!!$                   MAX(-pxtm1(1:kproma,:,jt)/time_step_len,pxtte(1:kproma,:,jt))
!!$           ! avoid negative tracer values
!!$           ELSEWHERE
!!$              pxtte(1:kproma,:,jt)=-pxtm1(1:kproma,:,jt)/time_step_len        
!!$           END WHERE
!!$        END IF
!!$     END DO
!!$  END IF
