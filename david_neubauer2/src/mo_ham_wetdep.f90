...
!>>DN: aerosol processing
  ...
  USE mo_tracdef,       ONLY: trlist, IN_CLOUD_LIQUID, IN_CLOUD_ICE
  USE mo_ham_m7_processing, ONLY: qwdep_ms4cd, qwdep_ms4ic, &
                                qcor_ms4cd, qcor_ms4ic, revap_vol
...
!>>DN: aerosol processing
  REAL(dp), PARAMETER :: astoso4 = 98.0734_dp/32.07_dp
  INTEGER :: ispec
  LOGICAL :: lincd,linic,lincloud
!<<DN: aerosol processing

  !--- 0/ Initializations:

  !>>DN: aerosol processing
  lincd = trlist%ti(kt)%nphase == IN_CLOUD_LIQUID
  linic = trlist%ti(kt)%nphase == IN_CLOUD_ICE
...
!<<DN: aerosol processing
  ...
  IF (laeroproc .AND. lstrat) THEN
     IF (lincd) THEN
        zxtice(1:kproma,:)=0._dp
        zxtwat(1:kproma,:)=pxtp1c(1:kproma,:,kt)
     ELSEIF (linic) THEN
        zxtice(1:kproma,:)=pxtp1c(1:kproma,:,kt)
        zxtwat(1:kproma,:)=0._dp
     ENDIF
  ENDIF
  !<<DN: aerosol processing
...
!>>DN: aerosol processing
  IF (laeroproc.AND.lstrat) THEN
     IF (lincloud) THEN
        ...
        WHERE(lincd .AND. trlist%ti(kt)%spid == id_so4 .AND. paclc(1:kproma,:) < zmin)                  &
             qcor_ms4cd(1:kproma,:,krow)=qcor_ms4cd(1:kproma,:,krow)                   &
             +MAX(0._dp,pxtp1c(1:kproma,:,kt))*prhop1(1:kproma,:)*delta_time/ztmst
        WHERE(linic .AND. trlist%ti(kt)%spid  == id_so4 .AND. paclc(1:kproma,:) < zmin)                  &
             qcor_ms4ic(1:kproma,:,krow)=qcor_ms4ic(1:kproma,:,krow)                   &
             +MAX(0._dp,pxtp1c(1:kproma,:,kt))*prhop1(1:kproma,:)*delta_time/ztmst

!set in-cloud aerosol zero if cloud cover is zero
        WHERE(paclc(1:kproma,:) < zmin) pxtp1c(1:kproma,:,kt)=0._dp              

        ispec = trlist%ti(kt)%spid
!        IF(ispec==id_so4)THEN
!          revap_vol(1:kproma,:,krow)=revap_vol(1:kproma,:,krow)+astoso4*&
!                (zdxtevapic(1:kproma,:) + zdxtevapbc(1:kproma,:))/pdpg(1:kproma,:)*ztmst/speclist(ispec)%density
!        ELSE
           revap_vol(1:kproma,:,krow)=revap_vol(1:kproma,:,krow)+&
                (zdxtevapic(1:kproma,:) + zdxtevapbc(1:kproma,:))/pdpg(1:kproma,:)*ztmst/speclist(ispec)%density
!        END IF
...
!     zxtte(1:kproma,:) = (pxtp10(1:kproma,:,kt)+pxtp1c(1:kproma,:,kt)-zxtp1(1:kproma,:)) / ztmst
     IF(.NOT.lincloud)THEN
        zxtte(1:kproma,:) = (pxtp10(1:kproma,:,kt)+pxtp1c(1:kproma,:,kt)-zxtp1(1:kproma,:)) / ztmst
     ELSE
        zxtte(1:kproma,:) = (pxtp1c(1:kproma,:,kt)-zxtp1(1:kproma,:)) / ztmst
     END IF
!<<DN: aerosol processing
...
!>>DN: aerosol processing
  IF (laeroproc.AND.lstrat) THEN
     IF (lincd .AND. trlist%ti(kt)%spid == id_so4) &
          qwdep_ms4cd(1:kproma,:,krow)=qwdep_ms4cd(1:kproma,:,krow) &
          +prhop1(1:kproma,:)*zxtte(1:kproma,:)*delta_time
     IF (linic .AND. trlist%ti(kt)%spid == id_so4) &
          qwdep_ms4ic(1:kproma,:,krow)=qwdep_ms4ic(1:kproma,:,krow) &
          +prhop1(1:kproma,:)*zxtte(1:kproma,:)*delta_time
  END IF
!<<DN: aerosol processing
...
!                        zxtfrac, zxtfrac_nuc, zxtfrac_imp)  
    IF (trlist%ti(kt)%nphase /= IN_CLOUD_LIQUID .AND. trlist%ti(kt)%nphase /= IN_CLOUD_ICE) THEN
       CALL get_icscavfrac(kproma, kbdim, klev, krow, ktop, kt,                &
                           kmod, kwat_phase, ktrac_phase, kscavICtype, lstrat, &
                           ld_wat, ld_mxp, ld_ice,                             &
                           prhop1, pxtp1c, pxtp1c_sav,                         &
...
!!$    pdxt(1:kproma,:)  = MERGE(ztmp1(1:kproma,:), 0._dp, ll1(1:kproma,:))
    IF (trlist%ti(kt)%nphase == IN_CLOUD_LIQUID .OR. trlist%ti(kt)%nphase == IN_CLOUD_ICE) THEN
       IF(laeroproc .AND. lstrat)THEN
          ztmp1(1:kproma,:)    = pxt(1:kproma,:)*1._dp*peff(1:kproma,:)
          pdxt_nuc(1:kproma,:) = MERGE(ztmp1(1:kproma,:), pxt(1:kproma,:), ll1(1:kproma,:))

          pdxt_imp(1:kproma,:) = 0._dp

          ztmp1(1:kproma,:) = pxt(1:kproma,:)*1._dp*peff(1:kproma,:)
          pdxt(1:kproma,:)  = MERGE(ztmp1(1:kproma,:), pxt(1:kproma,:), ll1(1:kproma,:))          
       END IF
    ELSE
       ztmp1(1:kproma,:)    = pxt(1:kproma,:)*zxtfrac_nuc(1:kproma,:)*peff(1:kproma,:)
       pdxt_nuc(1:kproma,:) = MERGE(ztmp1(1:kproma,:), 0._dp, ll1(1:kproma,:))
...
!>>DN: aerosol processing
!                         kscavICphase, kscavBCphase)
                         kscavICphase, kscavBCphase, lstrat)
!<<DN: aerosol processing
...
!  USE mo_tracdef, ONLY: trlist
  USE mo_tracdef, ONLY: trlist, AEROSOLNUMBER, AEROSOLMASS, IN_CLOUD_LIQUID, IN_CLOUD_ICE
  USE mo_ham,     ONLY: laeroproc
!<<DN: aerosol processing
...
!>>DN: aerosol processing
  IF(laeroproc)THEN
     IF ((trlist%ti(kt)%nphase == AEROSOLMASS .OR. trlist%ti(kt)%nphase == AEROSOLNUMBER)&
          .AND. lstrat) THEN
        kscavICtype  = 0
        kscavBCtype  = 2
        kscavICphase = 0
        kscavBCphase = 3
     ELSE IF((trlist%ti(kt)%nphase == IN_CLOUD_LIQUID .OR. trlist%ti(kt)%nphase == IN_CLOUD_ICE) &
          .AND. .NOT. lstrat) THEN
        kscavICtype  = 0
        kscavBCtype  = 0     
        kscavICphase = 0
