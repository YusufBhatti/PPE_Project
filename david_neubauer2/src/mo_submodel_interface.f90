...
!<<<mgs / --mgs 20140313
  ...
  USE mo_ham_m7_processing,ONLY: aeroproc_initialize
  !<<DN: aerosol processing
...
!>>DN: aerosol processing
  IF (laeroproc) CALL aeroproc_initialize
!<<DN: aerosol processing
#else
...
!!    USE mo_localtime,      ONLY: mo_localtime_init
    ...
    !>>DN: aerosol processing
    USE mo_ham_m7_processing,ONLY: construct_stream_aeroproc
    USE mo_ham,              ONLY: laeroproc
    !<<DN: aerosol processing
#endif
!!    USE mo_isccpsimulator,   ONLY: construct_stream_isccp
  ...
  ENDIF
    ...
    END IF
!>>DN: aerosol processing
    IF (laeroproc) CALL construct_stream_aeroproc
!<<DN: aerosol processing
#endif
...
!>>DN: aerosol processing
  USE mo_ham,               ONLY: laeroproc
  USE mo_ham_m7_processing, ONLY: aeroproc_interface
  USE mo_activ,             ONLY: dpg
!<<DN: aerosol processing
#endif
...
#ifdef HAMMOZ
!>>DN: aerosol processing
  IF (lham .AND. laeroproc) THEN
     dpg(1:kproma,:,krow) = zdpg(1:kproma,:)
     CALL aeroproc_interface(kbdim,  kproma, klev,  krow,                 &
                             paclc,  pxtm1,  pxtte,                       &   
                             paphp1, papp1, ptm1)
  END IF
!<<DN: aerosol processing
...
!>>DN convective mass flux diagnostics
  USE mo_activ,              ONLY: mc, mc_inst, w_conv, w_conv_inst
...
!>>DN convective mass flux diagnostics
  mc_inst(1:kproma,:,krow) = pmfu(1:kproma,:) + pmfd(1:kproma,:)
  mc(1:kproma,:,krow) = mc(1:kproma,:,krow) +  mc_inst(1:kproma,:,krow) * delta_time
  w_conv_inst(1:kproma,:,krow) = mc_inst(1:kproma,:,krow) / prhou(1:kproma,:)
  w_conv(1:kproma,:,krow) = w_conv(1:kproma,:,krow) +  w_conv_inst(1:kproma,:,krow) * delta_time
!<<DN
