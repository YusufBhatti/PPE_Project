...
!    USE mo_ham_streams, ONLY: frac, nact_strat, rc_strat
    USE mo_ham_streams, ONLY: frac, nact_strat, rc_strat, nact
!<<DN: aerosol processing
...
!>>DN: aerosol processing
    DO jclass=1, nclass
       nact(jclass)%ptr(1:kproma,:,krow)=pnact(1:kproma,:,jclass)
    END DO
!<<DN: aerosol processing
...
!    USE mo_ham_streams, ONLY: nact_conv, rc_conv
    USE mo_ham_streams, ONLY: nact_conv, rc_conv, nact
!<<DN: aerosol processing
    USE mo_param_switches, ONLY: nactivpdf
...
!    USE mo_ham_streams, ONLY: frac, nact_strat, nact_conv
    USE mo_ham_streams, ONLY: frac, nact_strat, nact_conv, nact
!<<DN: aerosol processing
    USE mo_tracdef,     ONLY: ntrac
...
!>>DN: aerosol processing
    DO jclass=1, nclass
       it= sizeclass(jclass)%idt_no
       nact(jclass)%ptr(1:kproma,:,krow)=0._dp
       WHERE(na(1:kproma,:,krow)>zeps) 
          nact(jclass)%ptr(1:kproma,:,krow)=pcdncact(1:kproma,:) &
                                          * pxtm1(1:kproma,:,it)*prho(1:kproma,:) &
                                          * frac(jclass)%ptr(1:kproma,:,krow) / na(1:kproma,:,krow)
       ENDWHERE
    END DO
!<<DN: aerosol processing
