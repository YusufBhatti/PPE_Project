...
!>>DN: aerosol processing
  REAL(dp), PUBLIC, POINTER :: d_prod_ms4cd(:,:)
!<<DN: aerosol processing   
...
!>>DN: aerosol processing
  TYPE (vmem3d), PUBLIC, ALLOCATABLE :: nact(:)
!<<DN: aerosol processing
...
!>>DN: aerosol processing
    IF (.NOT. ALLOCATED(nact)) ALLOCATE(nact(nclass))
!<<DN: aerosol processing
...
!>>DN: aerosol processing
    CALL add_stream_element (stream_ham, 'D_PROD_MS4CD',      d_prod_ms4cd,                                               &
                             longname='sulfate production liquid phase cd', units='kg m-2 s-1'               )
!<<DN: aerosol processing
...
!     (as for other quantities above, this is in fact still HAM-dep more than simply aerosol-dep)
    ...
    DO jclass=1, nclass
!>>DN: aerosol processing  
       CALL add_stream_element (stream_ham, 'NACT_'//TRIM(sizeclass(jclass)%shortname), nact(jclass)%ptr, &
                                longname='number of activated particles total', units='m-3')
!<<DN: aerosol processing
       !>>dod #377
