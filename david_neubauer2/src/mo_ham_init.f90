...
!                                    SOLUBLE, INSOLUBLE, t_flag, ln, ll
    ...
    USE mo_ham_species,       ONLY: id_so4, id_wat, idt_cd, idt_ic
...
!### not sure about this logic: please test!!
    ...
    !>>DN: aerosol processing
    !--- 6) Aerosol processing modes:
    IF (laeroproc) THEN
       DO jn = aero_idx(1),aero_idx(naerospec)

          IF (jn==id_wat) CYCLE

          trac_lname = TRIM(ADJUSTL(speclist(jn)%longname))//' in cloud droplet mass mixing ratio'         
          iburdenid = 0          
          IF (burden_keytype == BYTRACER) THEN       ! burden per species and per mode (i.e. per tracer)
             iburdenid = new_diag_burden(TRIM(ADJUSTL(speclist(jn)%shortname)) // '_CD',&
                                         itype=1, lclobber=.false.)
          ELSE IF (burden_keytype == BYSPECIES) THEN  ! burden per species
             iburdenid = new_diag_burden(speclist(jn)%shortname, itype=1, lclobber=.true.)
          END IF
         
          CALL new_tracer(TRIM(ADJUSTL(speclist(jn)%shortname)),'HAM',&
                          subname='CD',                               &
                          idx=idt_cd(jn),                             &                         
                          units=speclist(jn)%units,                   &
                          table=131,                                  &
                          code=igribc,                                &
                          nphase=IN_CLOUD_LIQUID,                     &                         
                          ndrydep=OFF,                                &
                          nwetdep=ON,                                 &
                          nsedi=OFF,                                  &
                          burdenid=iburdenid,                         &
          ...
          trac_lname = TRIM(ADJUSTL(speclist(jn)%longname))//' in ice crystal mass mixing ratio'
          iburdenid = 0          
          IF (burden_keytype == BYTRACER) THEN       ! burden per species and per mode (i.e. per tracer)
             iburdenid = new_diag_burden(TRIM(ADJUSTL(speclist(jn)%shortname)) // '_IC',&
                                         itype=1, lclobber=.false.)
          ELSE IF (burden_keytype == BYSPECIES) THEN  ! burden per species
             iburdenid = new_diag_burden(speclist(jn)%shortname, itype=1, lclobber=.true.)
          END IF
