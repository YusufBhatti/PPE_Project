...
!gf
  ...
  ! For in-cloud droplet aerosols and in-ice crystal aerosols: only the tracer index is needed,
  ! so an integer array is sufficient. The arrays are placed in this module to solve circular
  ! dependency problems. They are allocated by mo_aero_processing
  INTEGER, PUBLIC, ALLOCATABLE :: idt_cd(:)
  INTEGER, PUBLIC, ALLOCATABLE :: idt_ic(:)
  !<<DN: aerosol processing
