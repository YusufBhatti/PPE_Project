...
CONTAINS
  ...
  SUBROUTINE construct_stream_hifreq
    ...
    CALL add_stream_reference (hifreq, 'dpg','acheaci',lpost=.TRUE.) ! liquid Cloud Fraction 2D
    CALL add_stream_reference (hifreq, 'aclcov','g3b',lpost=.TRUE.) ! Cloud Fraction 2D
    CALL add_stream_reference (hifreq, 'aclcv','ham',lpost=.TRUE.) !
    CALL add_stream_reference (hifreq, 'relhum','echam',lpost=.TRUE.) !
    CALL add_stream_reference (hifreq, 'relhum','echamm',lpost=.TRUE.) !
    CALL add_stream_reference (hifreq, 'relhum','echam',lpost=.TRUE.) !
    CALL add_stream_reference (hifreq, 'relhum','ham',lpost=.TRUE.) !
