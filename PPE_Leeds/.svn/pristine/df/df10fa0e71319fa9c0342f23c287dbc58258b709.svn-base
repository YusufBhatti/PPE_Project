...
MODULE mo_hammoz_aerocom_HEmon
  ...
  REAL(dp), PUBLIC, POINTER :: pressure(:,:,:)
  ...
  SUBROUTINE construct_HEmon_stream
    ...
    CALL add_stream_element (achemon, 'clt', clt, &
     ...
     CALL add_stream_element (achemon, 'pressure', pressure, &
        longname = 'atmospheric_pressure', &
        units = 'Pa', &
        laccu = .TRUE., &
        lpost = .TRUE., &
        lrerun = .TRUE. )
  ...
  SUBROUTINE update_HEmon_diags(kproma, kbdim, klev, krow)
    ...
    !-- pressure
    pressure(1:kproma,:,krow) = pressure(1:kproma,:,krow) + vphysc%apm1(1:kproma,:,krow)*delta_time
