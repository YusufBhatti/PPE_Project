...
#----------------------------------------------------------------------------
# Sylvaine Ferrachat 2017-01:
# Rationale for this alternate symlinks template:
#
# This file is provided as support for users needing to run some specific intermediate versions
# between echam6.1.0-ham2.2.0-moz0.9.0 and echam6.3-ham2.3-moz1.0.
# --> search for 'Volcanic emissions' for full details.
#----------------------------------------------------------------------------
...
if $this_model_flag_with_hammoz ; then # echam6-hammoz specific
   ...
   if $flag_moz ; then
      ...
      echo "trac_init_date: $trac_init_date" | tee -a $log_file
   fi
fi
echo | tee -a $log_file
...
if $this_model_flag_with_hammoz ; then # echam6-hammoz specific
   ...
   #>>SF: Workaround for versions that are childs of any of the following range of trunk versions: [r2972:r3760]
   #      This comprises echam6.3.0-ham2.3-moz1.0_rc2 and echam6.3.0-ham2.3-moz1.0_rc3
   #
   #      Note that this purely solves a metadata pb in the 2 below input files, and bit-identity of the
   ...
   #-- Replaces 'v0002' by 'v0001' in $input_basepath
   alternate_basepath_volc=$(sed -e 's|v0002|v0001|' <<< "$input_basepath")

   ln -sf ${alternate_basepath_volc}/emissions_aerocom_II/explosive_volcanos.dat explosive_volcanos.dat
   ln -sf ${alternate_basepath_volc}/emissions_aerocom_II/continuous_volcanos.dat continuous_volcanos.dat
   #<<SF
   ...
   if $flag_moz ; then
      ...
      ln -sf  ${input_basepath}/hammoz/${hres}/moz_initial_${trac_init_date}_${hres}${vres}.nc tracer_ic.nc
