...
if $flag_nudg ; then
   ...
   while ((idate<=$stop_yyyymm)) ; do
       ...
       if $date_in_subdir_name ; then
          subdir_tail="_${hres}${vres}_${idate}${dir_ext}" # <-- no underscore at the beg (historical way)
       fi
       ...
       for prefix in ${nudg_prefix_order[*]} ; do
           ...
           nudg_subdir_attempt=${nudg_basepath}/${prefix}/${prefix}_${subdir_tail}
       ...
       if $flag_not_found ; then
	     ...
	     echo "Checking for nudging data in directory: ${nudg_subdir_attempt}"
