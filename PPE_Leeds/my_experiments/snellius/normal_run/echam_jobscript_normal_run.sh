...
if $flag_p_proc ; then # this submits a job to be executed on the p-proc machine julier.cscs.ch. 
   ...
   declare -x exp=normal_run
   declare -x exp_dir=/projects/0/prjs0937/yusufb/ECHAM/OUTPUT//snellius/DEBUG/normal_run/
   declare -x p_proc_dir=/path/to/post_proc_disk/ybhatti/normal_run
...
rerun_echam="restart_normal_run_echam.nc"

if [ -e ${rerun_echam} ] ; then
   rerun_date=`ncdump -h ${rerun_echam} | grep ":vdate =" | cut -c12-19`
   rerun_tar=restart_normal_run_${rerun_date}.tar

   tar cvf $rerun_tar restart_normal_run_[a-z]*
else
...
#--- Submit the next job:
   ...
   if [[ "$status_echam" -eq "0" ]] ; then
      cd /gpfs/work3/0/prjs0937/yusufb/Branches/PPE_Leeds/my_experiments/snellius/normal_run
      sbatch /gpfs/work3/0/prjs0937/yusufb/Branches/PPE_Leeds/my_experiments/snellius/normal_run/echam_jobscript_normal_run.sh
   fi
